        ...
        HStack {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .trailing, spacing: 2) {
                    ...
                    let mountSelected = driveManager.mountSelection.count
                    let unmountSelected = driveManager.unmountSelection.count
                    if mountSelected > 0 {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                // Get current drive state from driveManager
                InfoRow(label: "Selected for Mount:", value: driveManager.mountSelection.contains(drive.identifier) ? "Yes" : "No")
                InfoRow(label: "Selected for Unmount:", value: driveManager.unmountSelection.contains(drive.identifier) ? "Yes" : "No")
            }
            .padding()
            .background(Color.gray.opacity(0.05))
            .cornerRadius(10)
            ...
            HStack(spacing: 12) {
                ...
                if drive.isMounted {
                    ...
                    if !drive.mountPoint.contains("/System/Volumes/") &&
                       ...
                       !drive.mountPoint.contains("Library/Developer") {
                        ...
                        Button(action: {
                            ...
                            HStack {
                                let isSelected = driveManager.unmountSelection.contains(drive.identifier)
                                Image(systemName: isSelected ? "eject.circle.fill" : "eject.circle")
                                Text(isSelected ? "Deselect Unmount" : "Select to Unmount")
                            }
                            .frame(minWidth: 180)
                        }
                        .buttonStyle(.borderedProminent)
                        .tint(driveManager.unmountSelection.contains(drive.identifier) ? .orange : .blue)
                    } else {
                        Text("System Volume")
                            .foregroundColor(.secondary)
                            .frame(minWidth: 180)
                    ...
                    Button(action: {
                        ...
                        HStack {
                            let isSelected = driveManager.mountSelection.contains(drive.identifier)
                            Image(systemName: isSelected ? "play.circle.fill" : "play.circle")
                            Text(isSelected ? "Deselect Mount" : "Select to Mount")
                        }
                        .frame(minWidth: 180)
                    }
                    .buttonStyle(.borderedProminent)
                    .tint(driveManager.mountSelection.contains(drive.identifier) ? .green : .blue)
                }
