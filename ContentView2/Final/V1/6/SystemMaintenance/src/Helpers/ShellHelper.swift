...
struct ShellHelper {
    ...
    // Method to run commands with sudo using AppleScript
    static func runSudoCommand(_ command: String) -> (output: String, success: Bool) {
        print("🔧 Running sudo command: \(command)")
        
        // Use a safer approach with proper escaping
        // Replace single quotes with a safer sequence
        let safeCommand = command.replacingOccurrences(of: "'", with: "'\"'\"'")
        
        // Create a simple AppleScript that runs the command
        let appleScript = "do shell script \"\(safeCommand)\" with administrator privileges"
        
        // Escape the AppleScript itself
        let escapedAppleScript = appleScript.replacingOccurrences(of: "\"", with: "\\\"")
        
        // Run osascript with the escaped AppleScript
        let osascriptCommand = "osascript -e \"\(escapedAppleScript)\""
        
        return runCommand(osascriptCommand)
    }
