...
struct KextsManagerView: View {
    ...
    // Developer Info - Updated to Navaratnam Manoranjan
    @State private var developerName = "Navaratnam Manoranjan"
    @State private var developerEmail = "nmano0006@gmail.com"
    @State private var appVersion = "2.7.8.1.0"
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 4) {
                ...
                HStack {
                    ...
                    .buttonStyle(.plain)
                    .help("Copy email: \(developerEmail)")
                }
                .padding(.horizontal)
                .padding(.vertical, 6)
            }
    ...
    private func copyToClipboard(_ text: String) {
        ...
        alertMessage = "Copied to clipboard: \(text)"
        showInfoAlert = true
    }
    
    // MARK: - Author Badge View (updated for KextsManagerView)
    private struct AuthorBadgeView: View {
        let name: String
    ...
    private var InstallAppleHDAView: some View {
        ScrollView {
            VStack(spacing: 20) {
                // Developer Header for AppleHDA Tab - Updated
    ...
    private var KDKManagerView: some View {
        ScrollView {
            VStack(spacing: 20) {
                // Developer Header for KDK Tab - Updated
                DeveloperHeaderSection
    ...
    // MARK: - Developer Header Section
    private var DeveloperHeaderSection: some View {
        VStack(spacing: 8) {
            HStack {
            ...
            Text("For support: \(developerEmail)")
                .font(.caption2)
                .foregroundColor(.secondary)
        }
    }
    ...
    private var SIPInfoSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Developer note
            Text("\nNote: Developed by \(developerName)")
                .font(.caption2)
                .foregroundColor(.blue)
                .italic()
        }
    ...
    private var SystemInfoSection: some View {
        VStack(spacing: 12) {
            ...
            Text("macOS \(macOSVersion) (\(macOSBuild)) • Kernel \(getKernelVersion()) • App v\(appVersion)")
                .font(.caption)
                .foregroundColor(.secondary)
        }
        .padding()
    ...
    private var KDKStatusSection: some View {
        VStack(spacing: 12) {
            ...
            Text("Tool developed by \(developerName) - \(developerEmail)")
                .font(.caption2)
                .foregroundColor(.secondary)
                .padding(.top, 4)
        }
    ...
    private func StatusItem(title: String, status: String, version: String?, icon: String, color: Color) -> some View {
        VStack(spacing: 8) {
            ...
            Text(status)
                .font(.caption)
                .foregroundColor(status == "Installed" || status.contains("26.") || status == appVersion ? .green : .orange)
                .padding(.horizontal, 8)
                .padding(.vertical, 2)
                .background(
                    (status == "Installed" || status.contains("26.") || status == appVersion ? Color.green : Color.orange)
                        .opacity(0.1)
                )
                .cornerRadius(4)
    ...
    private var InstalledKDKsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Button(action: {
                    alertMessage = """
                    KDK Manager - OpenCore Configurator
                    
                    Developed by: \(developerName)
                    Version: \(appVersion)
                    Email: \(developerEmail)
                    
                    This tool helps manage Kernel Debug Kits for
                    kernel extension development on macOS.
                    
                    Features:
                    • KDK installation verification
                    • Compatibility checking
                    • Symbol creation for debugging
                    • Kext development support
    ...
    private var KDKActionsSection: some View {
        VStack(spacing: 12) {
            ...
            Button(action: {
                if let url = URL(string: "mailto:\(developerEmail)?subject=KDK%20Manager%20Support") {
                    NSWorkspace.shared.open(url)
                }
            }) {
                HStack {
    ...
    private var StatusOverviewSection: some View {
        VStack(spacing: 12) {
            ...
            Text("Audio tools by \(developerName) - \(developerEmail)")
                .font(.caption2)
                .foregroundColor(.secondary)
                .padding(.top, 4)
        }
        ...
        Group {
            if let efiPath = efiPath {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("Tool developed by \(developerName)")
                        .font(.caption2)
                        .foregroundColor(.secondary)
                        .padding(.top, 4)
                }
    ...
    private var AudioPackageSection: some View {
        VStack(spacing: 12) {
            HStack {
                ...
                // Developer contact button
                Button(action: {
                    if let url = URL(string: "mailto:\(developerEmail)?subject=AppleHDA%20Installation%20Help") {
                        NSWorkspace.shared.open(url)
                    }
                }) {
            ...
            Text("Developed by \(developerName) - v\(appVersion)")
                .font(.caption2)
                .foregroundColor(.secondary)
                .padding(.top, 4)
        }
    ...
    private var KextSourceSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Button(action: {
                    alertMessage = """
                    Kext Source Selection Help
                    
                    Developed by: \(developerName)
                    Version: \(appVersion)
                    
                    Select either:
                    1. A folder containing .kext files
                    2. A single .kext file
                    
                    Recommended kexts for audio:
                    • Lilu.kext
                    • AppleALC.kext
                    • AppleHDA.kext
        ...
        VStack(spacing: 12) {
            ...
            HStack(spacing: 12) {
                ...
                Button(action: {
                    ...
                    --------------------------
                    Tool developed by \(developerName)
                    Version: \(appVersion)
                    For help: \(developerEmail)
                    """
                    showInfoAlert = true
                }) {
                    HStack {
    ...
    private var EmptyStateView: some View {
        VStack(spacing: 12) {
            ...
            Text("Developer: \(developerName) - v\(appVersion)")
                .font(.caption2)
                .foregroundColor(.secondary)
                .padding(.top, 8)
        }
            ...
            ForEach(filteredKexts) { kext in
                KextRow(kext: kext)
                    .contextMenu {
                        ...
                        Button(action: {
                            alertMessage = """
                            Kext Manager - OpenCore Configurator
                            
                            Developed by: \(developerName)
                            Version: \(appVersion)
                            Email: \(developerEmail)
                            
                            For kernel extension management and
                            audio installation tools.
                            """
                            showInfoAlert = true
                        }) {
    ...
    private func verifyKDKCompatibility(_ kdk: KDKInfo) {
        ...
        messages.append("Tool Version: \(appVersion)")
        messages.append("Developer: \(developerName)")
    ...
    private func verifyKDKInstallation() {
        ...
        messages.append("\nFor support contact: \(developerEmail)")
    ...
    private func createKDKSymbols() {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            // Add developer info
            messages.append("Tool: OpenCore Configurator")
            messages.append("Version: \(self.appVersion)")
            messages.append("Developer: \(self.developerName)")
            ...
            do {
                ...
                Tool Information:
                • Tool: OpenCore Configurator
                • Version: \(self.appVersion)
                • Developer: \(self.developerName)
                • Email: \(self.developerEmail)
                ...
                # Generated by OpenCore Configurator v\(self.appVersion)
                # Developer: \(self.developerName)
                 ...
                 * Tool: OpenCore Configurator v\(self.appVersion)
                 * Developer: \(self.developerName)
                 */
                ...
                kern_return_t ExampleKext_start(kmod_info_t * ki, void *d) {
                    ...
                    printf("Tool: OpenCore Configurator v\(self.appVersion)\\n");
                    return KERN_SUCCESS;
                }
                ...
                ## Support
                For questions or support, contact: \(self.developerEmail)
                """
    ...
    private func installAudioPackage() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            messages.append("Tool: OpenCore Configurator v\(self.appVersion)")
            messages.append("Developer: \(self.developerName)")
            ...
            DispatchQueue.main.async {
                ...
                messages.append("\nTool: v\(self.appVersion)")
                messages.append("Developer: \(self.developerName)")
                messages.append("For support: \(self.developerEmail)")
    ...
    private func installWithAlternativeMethodInternal(sourceDir: String, efiPath: String) -> Bool {
        ...
        var messages: [String] = []
        messages.append("Tool: OpenCore Configurator v\(appVersion)")
        messages.append("Developer: \(developerName)")
    ...
    private func showKextInfo(_ kext: KextInfo) {
        var infoText = "Kext Information:\n\n"
        infoText += "Name: \(kext.name)\n"
    ...
    private func generateKextReport() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            report += "Tool: OpenCore Configurator\n"
            report += "Version: \(self.appVersion)\n"
            report += "Developer: \(self.developerName)\n"
            report += "Email: \(self.developerEmail)\n"
