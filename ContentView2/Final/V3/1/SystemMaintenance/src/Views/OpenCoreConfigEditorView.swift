...
class ConfigViewModel: ObservableObject {
    ...
    func saveConfig(to url: URL) {
        ...
        do {
            ...
            let serializableData = makeSerializable(configData)
            
            // Save as XML plist
            let data = try PropertyListSerialization.data(
                fromPropertyList: serializableData,
                format: .xml,
                options: 0
            )
    ...
    private func makeSerializable(_ value: Any) -> Any {
        ...
        case let dict as [String: Any]:
            return dict.mapValues { makeSerializable($0) }
        case let array as [Any]:
            return array.map { makeSerializable($0) }
    ...
    private func updateConfigData(for entry: ConfigEntry, with newValue: Any) {
        if entry.depth == 0 {
            ...
            // Nested entry - we need to find and update the parent
            updateNestedConfigData(for: entry, with: newValue)
        }
    }
    
    private func updateNestedConfigData(for entry: ConfigEntry, with newValue: Any) {
        // For now, let's use a simpler approach
        // We'll update the configData directly by traversing the path
        
        if let parentKey = entry.parentKey,
           var parentDict = configData[parentKey] as? [String: Any] {
            
            // Update the value in the parent dictionary
            parentDict[entry.key] = newValue
            configData[parentKey] = parentDict
            
            // Also update the parent entry in configEntries
            if let parentIndex = configEntries.firstIndex(where: { $0.key == parentKey && $0.depth == 0 }) {
                configEntries[parentIndex].actualValue = parentDict
                configEntries[parentIndex].value = "\(parentDict.count) keys"
            }
        }
    }
}
...
struct ConfigTableRow: View {
    ...
    private func createChildEntry(key: String, value: Any) -> ConfigEntry {
        let type: String
