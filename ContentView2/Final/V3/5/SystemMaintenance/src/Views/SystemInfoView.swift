...
struct SystemInfoView: View {
    ...
    private var DeveloperInfoSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                InfoRow(label: "Developer:", value: "Navaratnam Manoranjan")
                InfoRow(label: "Email:", value: "nmano0006@gmail.com")
    ...
    private var SupportInfoSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            SectionHeader(title: "Support This Project", icon: "heart.fill")
                .foregroundColor(.red)
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    ...
                    Text("Like this tool? Support my open-source work!")
                        .fontWeight(.medium)
                    
                    Spacer()
                }
                
                Text("Donations help fund testing devices, server costs, and ongoing maintenance for all my projects.")
                    .font(.caption)
    ...
    private func createExportContent() {
        ...
        exportContent += "DEVELOPER INFORMATION:\n"
        exportContent += "======================\n"
        exportContent += "Developer: Navaratnam Manoranjan\n"
        exportContent += "Email: nmano0006@gmail.com\n\n"
        
        // Support Information
        exportContent += "SUPPORT INFORMATION:\n"
        exportContent += "====================\n"
        exportContent += "Like this tool? Support my open-source work!\n"
        exportContent += "Donations help fund testing devices, server costs, and ongoing maintenance.\n"
        exportContent += "Donate via PayPal: https://www.paypal.com/donate/?business=H3PV9HX92AVMJ&no_recurring=0&item_name=Support+my+open-source+development+work.+Donations+help+fund+testing+devices%2C+server+costs%2C+and+ongoing+maintenance+for+all+my+projects.&currency_code=CAD\n"
        exportContent += "Thank you for your support!\n"
...
struct ExportSheetView: View {
    ...
    private func saveToDesktop() {
        guard !exportText.isEmpty else {
            saveAlertMessage = "❌ No data to export"
            showSaveAlert = true
            return
        }
        ...
        guard let window = NSApp.keyWindow ?? NSApp.mainWindow else {
            saveAlertMessage = "❌ Cannot find window to present save dialog"
            showSaveAlert = true
            isSaving = false
            return
        }
