...
class DriveManager: ObservableObject {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                ...
                self.allDrives = updatedDrives.filter { drive in
                    !drive.isEFI && !drive.name.contains("EFI") && 
    ...
    func toggleUnmountSelection(for drive: DriveInfo) {
        ...
        // Don't allow unmounting of internal drives
        guard !drive.isInternal else {
            print("⚠️ Cannot unmount internal drive")
            return
        }
    ...
    func selectAllForUnmount() {
        ...
        for drive in allDrives {
            let shouldSelectForUnmount = drive.isMounted && !drive.isInternal
