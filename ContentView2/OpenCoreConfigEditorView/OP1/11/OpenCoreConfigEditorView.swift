...
struct OpenCoreConfigEditorView: View {
    ...
    private func generateEntriesForSection(_ section: String) -> [ConfigEntry] {
        ...
        case "AppleInput":
            entries = [
                ConfigEntry(key: "Enabled", type: "Boolean", value: "false", isEnabled: false),
                ConfigEntry(key: "Protocol", type: "String", value: "Apple", isEnabled: true)
            ]
        case "Audio":
            entries = [
                ConfigEntry(key: "Enabled", type: "Boolean", value: "false", isEnabled: false),
                ConfigEntry(key: "Device", type: "String", value: "PciRoot(0x0)/Pci(0x1b,0x0)", isEnabled: true)
            ]
        case "Drivers":
            entries = [
                ConfigEntry(key: "OpenRuntime.efi", type: "Boolean", value: "true", isEnabled: true),
                ConfigEntry(key: "OpenCanopy.efi", type: "Boolean", value: "false", isEnabled: false)
            ]
        case "Input":
            entries = [
                ConfigEntry(key: "KeySupport", type: "Boolean", value: "false", isEnabled: false),
                ConfigEntry(key: "PointerSupport", type: "Boolean", value: "false", isEnabled: false)
            ]
        case "Output":
            entries = [
                ConfigEntry(key: "ConsoleMode", type: "String", value: "Max", isEnabled: true),
                ConfigEntry(key: "Resolution", type: "String", value: "Max", isEnabled: true)
            ]
    ...
    private func updateEntriesFromImportedConfig() {
        ...
        case "APFS":
            // Update APFS settings from config
            if let apfsConfig = configData["APFS"] as? [String: Any] {
                for setting in apfsSettings {
                    if let value = apfsConfig[setting] as? Bool {
                        apfsToggleStates[setting] = value
                    }
                }
            }
        ...
        default:
            // For other sections, check if they exist in config
            if let sectionData = configData[selectedSection] as? [String: Any] {
                let entryCount = sectionData.count
                configEntries = [
                    ConfigEntry(key: "Entries", type: "Dictionary", value: "\(entryCount) items", isEnabled: entryCount > 0)
                ]
            }
        }
    }
