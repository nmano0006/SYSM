...
"""
    ...
    private func loadSampleConfig() {
        // Load a simplified sample config based on your provided file
        let sampleConfig = """
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
...
struct SampleConfigLoaderView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text("Warning: This is a sample configuration for reference only. Do not use it directly without understanding each setting.")
                .font(.caption)
                .foregroundColor(.orange)
                .multilineTextAlignment(.center)
                .padding()
...
struct ConfigTreeItem: View {
                    ...
                    ForEach(0..<array.count, id: \.self) { index in
                        HStack {
                            ...
                            if let item = array[index] as? [String: Any] {
                                ...
                                Text(stringValueForItem(item: array[index]))
                                    .font(.system(.body, design: .monospaced))
                            }
                        }
                        .padding(.vertical, 2)
    ...
    private func stringValueForItem(item: Any) -> String {
        if let boolValue = item as? Bool {
            ...
            return stringValue
        } else if let dataValue = item as? Data {
