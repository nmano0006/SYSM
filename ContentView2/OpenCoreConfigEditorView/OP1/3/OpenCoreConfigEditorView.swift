...
import AppKit  // Added for NSColor
...
struct OpenCoreConfigEditorView: View {
    ...
    struct ConfigEditorView: View {
        ...
        var body: some View {
            if isEditing {
                ...
                ScrollView {
                    // Use regular Text instead of AttributedString for compatibility
    ...
    private func highlightSearch(in text: String) -> String {
        ...
        // Simple search highlighting by returning the text as-is
        // (We can't use NSAttributedString in SwiftUI Text directly on macOS)
        return text
    }
    ...
    private func formatXML(_ xml: String) -> String {
        ...
        // Replace all whitespace between tags with a single newline
        let regex = try? NSRegularExpression(pattern: pattern, options: [])
        let range = NSRange(location: 0, length: xml.utf16.count)
        let modified = regex?.stringByReplacingMatches(in: xml, options: [], range: range, withTemplate: ">\n<") ?? xml
        
        let lines = modified.components(separatedBy: "\n")
        ...
        for line in lines {
            ...
            if !trimmed.hasPrefix("</") && !trimmed.hasPrefix("<?") && !trimmed.hasPrefix("<!") && !trimmed.contains("/>") {
                if !trimmed.hasPrefix("<key>") && !trimmed.hasPrefix("<string>") && !trimmed.hasPrefix("<integer>") && !trimmed.hasPrefix("<true/>") && !trimmed.hasPrefix("<false/>") && !trimmed.hasPrefix("<data>") && !trimmed.hasPrefix("<date>") {
                    indentLevel += 1
                }
            }
        }
