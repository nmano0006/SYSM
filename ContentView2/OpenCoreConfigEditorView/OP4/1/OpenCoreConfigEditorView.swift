...
class ConfigViewModel: ObservableObject {
    ...
    private func convertValueToHashable(_ value: Any) -> AnyHashable {
        if let dict = value as? [String: Any] {
            ...
            return AnyHashable(string)  // Fixed: Explicitly convert String to AnyHashable
        ...
        return AnyHashable("Unknown")  // Fixed: Wrap string in AnyHashable
    }
    ...
    // Fixed: Changed from private to internal so it can be accessed from the view
    ...
    func saveConfig(to url: URL) {
        do {
            ...
            let data = try PropertyListSerialization.data(fromPropertyList: regularDict, format: .xml, options: 0)
                            ...
                            ForEach(viewModel.sections, id: \.self) { section in
                                if section == "---" {
                                    ...
                                    SectionRow(
                                        ...
                                        status: viewModel.getSectionStatus(section),  // Fixed: Now accessible
...
struct ConfigTableRow: View {
    ...
    private func generateChildEntries() {
        // Safely generate child entries without try-catch since no throwing code
