// Views/OpenCoreConfigEditorView.swift - Updated Version
import SwiftUI
...
struct OpenCoreConfigEditorView: View {
    ...
    private func scanForOpenCore() {
        isScanning = true
        
        DispatchQueue.global(qos: .userInitiated).async {
            let openCoreInfo = ShellHelper.detectOpenCore()
    ...
    private func generateEntriesForSection(_ section: String) -> [ConfigEntry] {
        ...
        if let dictData = sectionData as? [String: Any] {
            ...
            for (key, value) in dictData {
                ...
                let isEnabled = true
                let isOpenCoreSpecific = isOpenCoreSection(section)
            ...
            entries.append(ConfigEntry(
                ...
                isOpenCoreSpecific: isOpenCoreSection(section)
            ))
    ...
    private func generateDefaultEntriesForSection(_ section: String) -> [ConfigEntry] {
        ...
        default:
            entries = [
                ConfigEntry(key: "Enabled", type: "Boolean", value: "true", isEnabled: true, actualValue: nil, isOpenCoreSpecific: false)
            ]
        }
