...
struct ConfigTableRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .center) {
                if entry.type == "Dictionary" || entry.type == "Array" || entry.key == "Add" || entry.key == "Delete" || entry.key == "Patch" {
        ...
        Group {
            if entry.type == "Boolean" {
                ...
                Text(entry.value)
                    ...
                    .italic()
            } else if entry.key == "Add" || entry.key == "Delete" || entry.key == "Patch" {
                Text(entry.value)
                    .font(.system(size: 11, design: .monospaced))
                    .foregroundColor(.orange)
            } else {
    ...
    private func getValueCountString(for entry: ConfigEntry) -> String {
        if let dict = entry.actualValue as? [String: Any] {
            ...
            if let dict = entry.actualValue as? [String: Any], let array = dict[entry.key] as? [Any] {
                return "\(array.count) items"
            }
        }
        return ""
    ...
    var body: some View {
        if let dict = entry.actualValue as? [String: Any] {
            if entry.key == "Add" || entry.key == "Delete" || entry.key == "Patch" {
                ACPIEntriesView(entries: dict[entry.key] as? [Any] ?? [], entryKey: entry.key)
...
struct ACPIEntriesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
            ForEach(0..<min(entries.count, 25), id: \.self) { index in
                if let entry = entries[index] as? [String: Any] {
                    HStack {
                        ...
                        if let comment = entry["Comment"] as? String {
                            Text(comment)
                                .font(.system(size: 9))
                                .foregroundColor(.gray)
                                .lineLimit(1)
            ...
            if entries.count > 25 {
                Text("... and \(entries.count - 25) more items")
                    .font(.system(size: 9))
                    .foregroundColor(.gray)
                    .padding(.leading, 32)
                    .padding(.top, 4)
...
struct OpenCoreConfigEditorView: View {
    ...
    private func loadDefaultConfig() {
        ...
        let defaultACPIEntries = [
            ...
            ["Enabled": true, "Path": "SSDT-ICPU.aml", "Comment": "CPU Alternative"],
            ["Enabled": false, "Path": "SSDT-Disable-ICPU.aml", "Comment": "Disable CPU"],
            ["Enabled": true, "Path": "SSDT-MEI.aml", "Comment": "MEI Device"],
            ["Enabled": true, "Path": "SSDT-RP01.aml", "Comment": "PCI Slot 1"],
            ["Enabled": true, "Path": "SSDT-RP01-VTDWiFi.aml", "Comment": "Wi-Fi Slot"],
            ["Enabled": true, "Path": "SSDT-RP03.aml", "Comment": "PCI Slot 3"],
            ["Enabled": true, "Path": "SSDT-RP05.aml", "Comment": "PCI Slot 5"],
            ["Enabled": true, "Path": "SSDT-RP09-TB3-HackInDROM.aml", "Comment": "Thunderbolt"],
            ["Enabled": true, "Path": "SSDT-Bridge.aml", "Comment": "Bridge Device"],
            ["Enabled": true, "Path": "SSDT-DTPG.aml", "Comment": "DTPG Fix"],
            ["Enabled": true, "Path": "SSDT-EC.aml", "Comment": "Embedded Controller"],
            ["Enabled": true, "Path": "SSDT-H_EC.aml", "Comment": "EC Alternative"],
            ["Enabled": true, "Path": "SSDT-HPET.aml", "Comment": "HPET Fix"],
            ["Enabled": true, "Path": "SSDT-PEGO-SSD970-SSD0.aml", "Comment": "NVMe SSD"],
            ["Enabled": true, "Path": "SSDT-PC00-Z790.aml", "Comment": "Motherboard"],
            ["Enabled": true, "Path": "SSDT-PMC.aml", "Comment": "Power Management"],
            ["Enabled": true, "Path": "SSDT-RTC-AWAC.aml", "Comment": "AWAC Fix"],
            ["Enabled": true, "Path": "SSDT-SBUS-MCHC.aml", "Comment": "SMBUS Fix"]
        ]
