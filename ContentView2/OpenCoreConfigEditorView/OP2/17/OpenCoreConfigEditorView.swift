...
struct ConfigTableRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .center) {
                ...
                if entry.isExpandable {
                    Button(action: {
                        ...
                        Image(systemName: expandedValues.contains(entry.id) ? "chevron.down" : "chevron.right")
                            .font(.system(size: 10))
                            .frame(width: 16)
                    }
                    .buttonStyle(.plain)
            ...
            if expandedValues.contains(entry.id) && !entry.childEntries.isEmpty {
                ForEach(entry.childEntries) { childEntry in
                    ConfigTableRow(
                        entry: childEntry,
                        isEditing: isEditing,
                        expandedValues: $expandedValues,
    ...
    private func getValueCountString(for entry: ConfigEntry) -> String {
        ...
        } else if let actualValue = entry.actualValue {
            // Handle ACPI and Kext entries
            if let dict = actualValue as? [String: Any],
               let array = dict[entry.key] as? [Any] {
                return "\(array.count) items"
            }
        }
        return ""
...
struct OpenCoreConfigEditorView: View {
    ...
    private func generateEntriesFromValue(value: Any, key: String, parentKey: String?, isOpenCoreSpecific: Bool, depth: Int) -> [ConfigEntry] {
        var entries: [ConfigEntry] = []
        
        if let dict = value as? [String: Any] {
            addDebugLog("  Dictionary '\(key)' with \(dict.count) items")
            
            let isExpandable = dict.count > 0
            ...
            if isExpandable {
                for (subKey, subValue) in dict.sorted(by: { $0.key < $1.key }) {
                    ...
                    // Store child entries in parent
                    if let index = entries.firstIndex(where: { $0.key == key }) {
                        var updatedEntry = entries[index]
                        updatedEntry.childEntries = childEntries
            ...
            let isExpandable = array.count > 0
            ...
            // Generate child entries for arrays (especially for ACPI/Kexts/Drivers)
            if isExpandable && (key == "Add" || key == "Delete" || key == "Patch" || key == "Kexts" || key == "Drivers") {
                var childEntries: [ConfigEntry] = []
                
                for (index, item) in array.enumerated() {
                    let itemKey = "Item \(index + 1)"
                    
                    if let dictItem = item as? [String: Any] {
                        let childEntry = ConfigEntry(
                            key: itemKey,
                            type: "Dictionary",
                            value: getArrayItemDescription(dictItem),
                            isEnabled: dictItem["Enabled"] as? Bool ?? false,
                            actualValue: dictItem,
                            isOpenCoreSpecific: isOpenCoreSpecific,
                            parentKey: key,
                            depth: depth + 1,
                            isExpandable: false,
                        ...
                        let childEntry = ConfigEntry(
                            key: itemKey,
                            type: getTypeString(for: item),
                            value: getValueString(for: item, type: getTypeString(for: item)),
                            isEnabled: true,
                            actualValue: item,
                            isOpenCoreSpecific: isOpenCoreSpecific,
                            parentKey: key,
    ...
    private func getArrayItemDescription(_ dict: [String: Any]) -> String {
        if let path = dict["Path"] as? String {
            let fileName = URL(fileURLWithPath: path).lastPathComponent
            let enabled = dict["Enabled"] as? Bool ?? false
            return "\(fileName) [\(enabled ? "Enabled" : "Disabled")]"
        } else if let bundlePath = dict["BundlePath"] as? String {
            let fileName = URL(fileURLWithPath: bundlePath).lastPathComponent
            let enabled = dict["Enabled"] as? Bool ?? false
            return "\(fileName) [\(enabled ? "Enabled" : "Disabled")]"
        } else if let comment = dict["Comment"] as? String {
            let enabled = dict["Enabled"] as? Bool ?? false
            return "\(comment) [\(enabled ? "Enabled" : "Disabled")]"
        } else {
    ...
    private func generateDefaultEntriesForSection(_ section: String) -> [ConfigEntry] {
        ...
        case "ACPI":
            entries = [
                ConfigEntry(key: "Add", type: "Array", value: "0 items", isEnabled: true, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Delete", type: "Array", value: "0 items", isEnabled: false, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Patch", type: "Array", value: "0 items", isEnabled: true, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Quirks", type: "Dictionary", value: "0 settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "Booter":
            entries = [
                ConfigEntry(key: "MmioWhitelist", type: "Array", value: "Items", isEnabled: false, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Patch", type: "Array", value: "Items", isEnabled: false, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Quirks", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "DeviceProperties":
            entries = [
                ConfigEntry(key: "Add", type: "Dictionary", value: "Properties", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Delete", type: "Dictionary", value: "Properties", isEnabled: false, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "Kernel":
            entries = [
                ConfigEntry(key: "Add", type: "Array", value: "0 items", isEnabled: true, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Block", type: "Array", value: "0 items", isEnabled: false, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Patch", type: "Array", value: "0 items", isEnabled: true, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Quirks", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Scheme", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "Misc":
            entries = [
                ConfigEntry(key: "Boot", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Debug", type: "Dictionary", value: "Settings", isEnabled: false, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Security", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Tools", type: "Array", value: "Tools", isEnabled: true, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "NVRAM":
            entries = [
                ConfigEntry(key: "Add", type: "Dictionary", value: "Variables", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Delete", type: "Dictionary", value: "Variables", isEnabled: false, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "LegacySchema", type: "Dictionary", value: "Settings", isEnabled: false, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "WriteFlash", type: "Boolean", value: "true", isEnabled: true, actualValue: nil, isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "PlatformInfo":
            entries = [
                ConfigEntry(key: "Automatic", type: "Boolean", value: "false", isEnabled: false, actualValue: nil, isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Generic", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "UpdateDataHub", type: "Boolean", value: "true", isEnabled: true, actualValue: nil, isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "UpdateSMBIOS", type: "Boolean", value: "true", isEnabled: true, actualValue: nil, isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "UEFI":
            entries = [
                ConfigEntry(key: "APFS", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "ConnectDrivers", type: "Boolean", value: "true", isEnabled: true, actualValue: nil, isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Drivers", type: "Array", value: "0 items", isEnabled: true, actualValue: [], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Input", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Output", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Quirks", type: "Dictionary", value: "Settings", isEnabled: true, actualValue: [:], isOpenCoreSpecific: true, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "AppleInput":
            entries = [
                ConfigEntry(key: "AppleEvent", type: "String", value: "Builtin", isEnabled: true, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "CustomDelays", type: "Boolean", value: "false", isEnabled: false, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "Audio":
            entries = [
                ConfigEntry(key: "AudioSupport", type: "Boolean", value: "false", isEnabled: false, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "PlayChime", type: "String", value: "Auto", isEnabled: true, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "Drivers":
            entries = [
                ConfigEntry(key: "OpenRuntime.efi", type: "Boolean", value: "true", isEnabled: true, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "HfsPlus.efi", type: "Boolean", value: "true", isEnabled: true, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "Input":
            entries = [
                ConfigEntry(key: "KeySupport", type: "Boolean", value: "false", isEnabled: false, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "PointerSupport", type: "Boolean", value: "false", isEnabled: false, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false)
            ]
        case "Output":
            entries = [
                ConfigEntry(key: "ConsoleMode", type: "String", value: "Max", isEnabled: true, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false),
                ConfigEntry(key: "Resolution", type: "String", value: "Max", isEnabled: true, actualValue: nil, isOpenCoreSpecific: false, depth: 0, isExpandable: false, isExpanded: false)
            ]
        default:
            entries = [
                ConfigEntry(key: "Enabled", type: "Boolean", value: "true", isEnabled: true, actualValue: nil, isOpenCoreSpecific: isOpenCoreSpecificValue, depth: 0, isExpandable: false, isExpanded: false)
            ]
        }
