...
struct OpenCoreConfigEditorView: View {
    ...
    private func getValueString(for value: Any, type: String) -> String {
        ...
        case let string as String:
            return string.prefix(50).string // Limit string length
    ...
    private func loadConfig(url: URL) {
        ...
        do {
            ...
            if let plist = try PropertyListSerialization.propertyList(from: data, options: [], format: nil) as? [String: Any] {
                ...
                // Log ACPI section for debugging
                if let acpi = configData["ACPI"] as? [String: Any] {
                    print("📊 ACPI section has keys: \(acpi.keys.joined(separator: ", "))")
                    if let addArray = acpi["Add"] as? [Any] {
                        print("📊 ACPI.Add has \(addArray.count) items")
                    }
                }
