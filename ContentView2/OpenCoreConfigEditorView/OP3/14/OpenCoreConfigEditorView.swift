...
struct OpenCoreConfigEditorView: View {
    ...
    @State private var configTitle = "OpenCore Configurator"
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 0) {
                    HStack {
                        ...
                        Button(action: {
                            isEditing.toggle()
                            alertMessage = isEditing ? "Entered edit mode" : "Exited edit mode"
                            showAlert = true
                        }) {
                            HStack {
    ...
    private func filterEntries() {
        guard !searchText.isEmpty else {
            // Reset to show all entries when search is cleared
            updateConfigEntriesForSection(selectedSection)
            return
        }
        ...
        for entry in entries {
            ...
            if matches {
                ...
                if !filteredChildren.isEmpty {
                    // Some children match, keep this parent and expand it
...
struct ConfigTableRow: View {
    ...
    private func generateChildEntries(for parentEntry: ConfigEntry) -> [ConfigEntry] {
        ...
        // Use safe casting and handle errors
        do {
            if let dict = parentEntry.actualValue as? [String: Any] {
                for (key, value) in dict.sorted(by: { $0.key < $1.key }) {
                    let childEntry = createConfigEntry(
            ...
            print("Error generating child entries: \(error)")
        }
    ...
    private func createConfigEntry(key: String, value: Any, parentKey: String, depth: Int, isOpenCoreSpecific: Bool) -> ConfigEntry {
        ...
        case is String:
            type = "String"
            valueString = value as? String ?? "Unknown"
            isEnabled = true
            isExpandable = false
        case is Bool:
            type = "Boolean"
            valueString = (value as? Bool ?? false) ? "true" : "false"
            isEnabled = value as? Bool ?? false
            isExpandable = false
        ...
        case is Double, is Float:
            type = "Double"
            valueString = String(format: "%.2f", value as? Double ?? 0.0)
            isEnabled = true
            isExpandable = false
        case let dict as [String: Any]:
            type = "Dictionary"
        ...
        case is Date:
            ...
            let formatter = DateFormatter()
            formatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
            valueString = formatter.string(from: value as? Date ?? Date())
            isEnabled = true
            isExpandable = false
        default:
            type = "Unknown"
