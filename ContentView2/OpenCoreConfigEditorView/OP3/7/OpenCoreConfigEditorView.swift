...
class ConfigStateManager: ObservableObject {
    @Published var configData: [String: Any] = [:]
    @Published var isEditing = false
    @Published var selectedSection = "ACPI"
    @Published var configEntries: [ConfigEntry] = []
    @Published var expandedEntries: Set<UUID> = []
    
    private var sectionDataCache: [String: [ConfigEntry]] = [:]
    ...
    func updateConfigEntriesForSection(_ section: String) {
        if let cached = sectionDataCache[section] {
            configEntries = cached
            return
        }
        
        let entries = generateEntriesFromConfigData(section: section)
        configEntries = entries
        sectionDataCache[section] = entries
...
// MARK: - Main View with Editing - Simplified
struct OpenCoreConfigEditorView: View {
    @StateObject private var configState = ConfigStateManager()
    ...
    var body: some View {
        NavigationView {
            SidebarView(
                ...
                showOpenCoreInfo: $showOpenCoreInfo,
                openCoreInfo: openCoreInfo,
                showDebugInfo: $showDebugInfo,
                configData: configState.configData,
                apfsSettings: apfsSettings,
                apfsToggleStates: $apfsToggleStates,
                apfsTextValues: $apfsTextValues,
                sections: sections,
                selectedSection: $configState.selectedSection,
                quirks: quirks,
                quirksToggleStates: $quirksToggleStates,
                isEditing: configState.isEditing,
                getSectionStatus: getSectionStatus,
                getActualEntryCount: getActualEntryCount,
                isOpenCoreSection: isOpenCoreSection,
                updateSection: { section in
                    configState.selectedSection = section
                    configState.updateConfigEntriesForSection(section)
                }
            )
            ...
            MainContentView(
                ...
                showAlert: $showAlert,
                scanForOpenCore: scanForOpenCore,
                showRawJSONView: showRawJSONView,
                importConfig: importConfig,
                exportConfig: exportConfig,
                updateConfigEntry: updateConfigEntry,
                addNewEntry: addNewEntry,
                saveChangesToConfigData: saveChangesToConfigData,
                ocVersion: ocVersion,
                configTitle: $configTitle,
                openCoreInfo: $openCoreInfo,
                filePathBinding: $filePath
            )
        }
        .navigationTitle("")
        ...
        .onAppear {
            loadDefaultConfig()
            configState.updateConfigEntriesForSection(configState.selectedSection)
    ...
    private func showRawJSONView() {
        do {
            let jsonData = try JSONSerialization.data(withJSONObject: configState.configData, options: [.prettyPrinted, .sortedKeys])
    ...
    private func updateConfigEntry(entry: ConfigEntry, newKey: String, newValue: Any?) {
        if let index = configState.configEntries.firstIndex(where: { $0.id == entry.id }) {
            var updatedEntry = configState.configEntries[index]
            
            if newKey.isEmpty {
                configState.configEntries.remove(at: index)
                ...
                configState.configEntries[index] = updatedEntry
            }
        }
    }
    
    private func updateConfigData(entry: ConfigEntry) {
        guard let parentKey = entry.parentKey else { return }
        
        if var sectionData = configState.configData[parentKey] as? [String: Any] {
            sectionData[entry.key] = entry.actualValue
            configState.configData[parentKey] = sectionData
            configState.updateConfigEntriesForSection(parentKey)
        }
    }
    ...
    private func removeFromConfigData(entry: ConfigEntry) {
        ...
        if var sectionData = configState.configData[parentKey] as? [String: Any] {
            sectionData.removeValue(forKey: entry.key)
            configState.configData[parentKey] = sectionData
            configState.updateConfigEntriesForSection(parentKey)
        }
    }
    ...
    private func addNewEntry(key: String, type: String, value: String) {
        ...
        let newEntry = ConfigEntry(
            ...
            isOpenCoreSpecific: false,
            parentKey: configState.selectedSection,
            depth: 1,
            isExpandable: type == "Array" || type == "Dictionary"
        )
        
        configState.configEntries.append(newEntry)
        
        if var sectionData = configState.configData[configState.selectedSection] as? [String: Any] {
            sectionData[key] = actualValue
            configState.configData[configState.selectedSection] = sectionData
        }
    }
        ...
        await MainActor.run {
            ...
            if let info = openCoreInfo {
                ...
                if let config = ShellHelper.getOpenCoreConfig() {
                    self.configState.configData = config
                    self.configState.updateConfigEntriesForSection(self.configState.selectedSection)
    ...
    private func getSectionStatus(_ section: String) -> String {
        if configState.configData[section] != nil {
            return "Active"
        }
    ...
    private func getActualEntryCount(for section: String) -> Int {
        if let sectionData = configState.configData[section] as? [String: Any] {
            if section == "ACPI" {
                var total = 0
        ...
        } else if let sectionData = configState.configData[section] as? [Any] {
            return sectionData.count
        }
        ...
        do {
            ...
            if let plist = try PropertyListSerialization.propertyList(from: data, options: [], format: nil) as? [String: Any] {
                ...
                alertMessage = "Configuration loaded successfully from: \(url.lastPathComponent)\n" +
                              "Found \(configState.configData.count) sections\n" +
    ...
    private func getACPIEntryCount(for subsection: String) -> Int {
        guard let acpiSection = configState.configData["ACPI"] as? [String: Any],
    ...
    private func getKernelEntryCount(for subsection: String) -> Int {
        guard let kernelSection = configState.configData["Kernel"] as? [String: Any],
    ...
    private func getUEFIEntryCount(for subsection: String) -> Int {
        guard let uefiSection = configState.configData["UEFI"] as? [String: Any],
    ...
    private func saveConfig(to url: URL) {
        do {
            let data = try PropertyListSerialization.data(fromPropertyList: configState.configData, format: .xml, options: 0)
...
// MARK: - Sidebar View (Extracted)
struct SidebarView: View {
    let configTitle: String
    @Binding var showOpenCoreInfo: Bool
    let openCoreInfo: OpenCoreInfo?
    @Binding var showDebugInfo: Bool
    let configData: [String: Any]
    let apfsSettings: [String]
    @Binding var apfsToggleStates: [String: Bool]
    @Binding var apfsTextValues: [String: String]
    let sections: [String]
    @Binding var selectedSection: String
    let quirks: [String]
    @Binding var quirksToggleStates: [String: Bool]
    let isEditing: Bool
    let getSectionStatus: (String) -> String
    let getActualEntryCount: (String) -> Int
    let isOpenCoreSection: (String) -> Bool
    let updateSection: (String) -> Void
                        ...
                        ForEach(sections, id: \.self) { section in
                            if section == "---" {
                                ...
                                SectionRow(
                                    ...
                                    entryCount: getActualEntryCount(section),
...
// MARK: - Main Content View (Extracted)
struct MainContentView: View {
    @ObservedObject var configState: ConfigStateManager
    ...
    @Binding var selectedEntryForDetail: ConfigEntry?
    let isEditing: Bool
    @Binding var alertMessage: String
    @Binding var showAlert: Bool
    let scanForOpenCore: () async -> Void
    let showRawJSONView: () -> Void
    let importConfig: () -> Void
    let exportConfig: () -> Void
    let updateConfigEntry: (ConfigEntry, String, Any?) -> Void
    let addNewEntry: (String, String, String) -> Void
    let saveChangesToConfigData: () -> Void
    let ocVersion: String
    @Binding var configTitle: String
    @Binding var openCoreInfo: OpenCoreInfo?
    @Binding var filePathBinding: String
    ...
    var body: some View {
        VStack(spacing: 0) {
            VStack(spacing: 0) {
                HStack {
                    ...
                    Button(action: {
                        ...
                        configState.isEditing.toggle()
                        showAlert = true
                    }) {
                        HStack {
                            Image(systemName: configState.isEditing ? "checkmark.circle.fill" : "pencil")
                                .font(.system(size: 11))
                            Text(configState.isEditing ? "Save" : "Edit")
                                .font(.system(size: 11))
                        }
                    }
                    .buttonStyle(.borderedProminent)
            ...
            if configData.isEmpty {
                ...
                ConfigTableView(
                    entries: $configState.configEntries,
                    isEditing: $configState.isEditing,
                    expandedEntries: $configState.expandedEntries,
                    onEntryTap: { entry in
                        selectedEntryForDetail = entry
                        showDetailView = true
                    },
                    onEntryChange: { entry, newKey, newValue in
                        updateConfigEntry(entry, newKey, newValue)
                    }
                )
            }
        }
...
// MARK: - Add Entry Sheet (Extracted)
struct AddEntrySheet: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            Text("Add New Entry to \(selectedSection)")
                .font(.headline)
