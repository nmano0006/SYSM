...
struct SSDTGeneratorView: View {
    ...
    private func saveToFile() {
        ...
        panel.title = "Save SSDTs"
        panel.nameFieldStringValue = "SSDTs.zip"
        panel.canCreateDirectories = true
        
        if panel.runModal() == .OK, let url = panel.url {
            saveSSDTsToFile(at: url)
        }
    }
    
    private func saveSSDTsToFile(at url: URL) {
        Task {
            let outputDir = getOutputDirectory()
            let fileManager = FileManager.default
            ...
            do {
                ...
                let files = try fileManager.contentsOfDirectory(atPath: outputDir)
                let generatedFiles = files.filter { $0.hasSuffix(".dsl") || $0.hasSuffix(".aml") }
                
                for file in generatedFiles {
                    let sourcePath = "\(outputDir)/\(file)"
                    let destPath = tempDir.appendingPathComponent(file)
                    try fileManager.copyItem(at: URL(fileURLWithPath: sourcePath), to: destPath)
                }
                ...
                let readmePath = tempDir.appendingPathComponent("README.md")
                try readmeContent.write(to: readmePath, atomically: true, encoding: .utf8)
                
                // Zip the directory
                let zipPath = url.deletingPathExtension().appendingPathExtension("zip")
                let zipResult = ShellHelper.runCommand("cd \"\(tempDir.path)\" && zip -r \"\(zipPath.path)\" .")
                
                await MainActor.run {
                    if zipResult.success {
                        alertTitle = "Save Successful"
                        alertMessage = "SSDTs saved to: \(zipPath.path)"
                        showAlert = true
                    } else {
                        alertTitle = "Save Failed"
                        alertMessage = "Failed to create zip file: \(zipResult.output)"
                        showAlert = true
                    }
                }
