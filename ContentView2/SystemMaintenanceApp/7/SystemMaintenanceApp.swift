...
struct SystemMaintenanceView: View {
    ...
    private var contentView: some View {
        Group {
            ...
            case 1:
                OpenCoreConfigEditorViewWrapper()
            case 2:
...
// MARK: - Wrapper for OpenCoreConfigEditorView to avoid scoping issues
struct OpenCoreConfigEditorViewWrapper: View {
    var body: some View {
        if #available(macOS 11.0, *) {
            OpenCoreConfigEditorView()
                .frame(maxWidth: .infinity, maxHeight: .infinity)
        } else {
            Text("OpenCore Config Editor requires macOS 11.0 or later")
                .frame(maxWidth: .infinity, maxHeight: .infinity)
        }
    }
}
