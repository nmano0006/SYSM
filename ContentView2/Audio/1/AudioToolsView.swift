...
struct AudioToolsView: View {
    ...
    private func setupAudioSession() {
        ...
        // We'll use shell commands to control audio instead
        print("Setting up audio for macOS")
    }
    ...
    private func refreshAudioDevices() {
        ...
        system_profiler SPAudioDataType -xml 2>/dev/null | \
        xpath -q -e '//dict/key[text()="_items"]/following-sibling::array[1]/dict' 2>/dev/null | \
        while read -r line; do
            if [[ "$line" == *"<dict>"* ]]; then
                ...
                is_default_output="0"
                is_default_input="0"
            elif [[ "$line" == *"<key>_name</key>"* ]]; then
                read -r next_line
                device_name=$(echo "$next_line" | sed 's/<string>//;s/<\/string>//')
            elif [[ "$line" == *"coreaudio_device_uid</key>"* ]]; then
                read -r next_line
                device_uid=$(echo "$next_line" | sed 's/<string>//;s/<\/string>//')
            elif [[ "$line" == *"coreaudio_output_uid</key>"* ]]; then
                read -r next_line
                output_uid=$(echo "$next_line" | sed 's/<string>//;s/<\/string>//')
            elif [[ "$line" == *"coreaudio_input_uid</key>"* ]]; then
                read -r next_line
                input_uid=$(echo "$next_line" | sed 's/<string>//;s/<\/string>//')
            elif [[ "$line" == *"coreaudio_default_sample_rate</key>"* ]]; then
                read -r next_line
                sample_rate=$(echo "$next_line" | sed 's/<string>//;s/<\/string>//')
            elif [[ "$line" == *"coreaudio_device_transport</key>"* ]]; then
                read -r next_line
                transport=$(echo "$next_line" | sed 's/<string>//;s/<\/string>//')
            elif [[ "$line" == *"coreaudio_default_audio_output_device</key>"* ]]; then
                read -r next_line
                if [[ "$next_line" == *"spaudio_yes"* ]]; then
                    is_default_output="1"
                fi
            elif [[ "$line" == *"coreaudio_default_audio_input_device</key>"* ]]; then
                read -r next_line
                if [[ "$next_line" == *"spaudio_yes"* ]]; then
                    is_default_input="1"
                fi
                ...
                if [[ -n "$device_name" ]]; then
                    echo "$device_name|$device_uid|$output_uid|$input_uid|$sample_rate|$transport|$is_default_output|$is_default_input"
                fi
            fi
        done
        """)
        ...
        for line in lines {
            ...
            if components.count >= 8 {
                ...
                let outputUid = components[2]
                let inputUid = components[3]
                let sampleRate = components[4]
                let transport = components[5]
                let isDefaultOutput = components[6] == "1"
                let isDefaultInput = components[7] == "1"
                
                // Determine device type
                let isInput = !inputUid.isEmpty
                let isOutput = !outputUid.isEmpty
                let deviceType: AudioDeviceType = isInput && isOutput ? .both : (isInput ? .input : .output)
                
                // Determine if current device
                let isCurrentOutput = isDefaultOutput
                let isCurrentInput = isDefaultInput
        ...
        // Fallback if no devices found or command failed
        if devices.isEmpty {
            print("⚠️ Using fallback audio device detection")
            
            // Try simpler command
            let simpleResult = shellHelper.runCommand("""
            system_profiler SPAudioDataType | grep -E '(^        _name:|coreaudio_device_uid:|coreaudio_default_audio_output_device:|coreaudio_default_audio_input_device:)' | \
            sed 's/^        _name: //;s/^            coreaudio_device_uid: //;s/^            coreaudio_default_audio_output_device: //;s/^            coreaudio_default_audio_input_device: //' | \
            tr '\\n' '|' | sed 's/||/\\n/g' | grep -v '^$'
            """)
            
            let simpleLines = simpleResult.output.components(separatedBy: "\n").filter { !$0.isEmpty }
            ...
            for line in simpleLines {
                ...
                if components.count >= 2 {
                    ...
                    let isDefaultOutput = components.contains { $0.contains("spaudio_yes") }
                    let isDefaultInput = components.contains { $0.contains("spaudio_yes") }
                    
                    let device = AudioDevice(
                        id: uid,
                        name: name,
                        uid: uid,
                        type: .both, // Assume both for fallback
                        sampleRate: "44100",
                        channels: "2",
                        isDefault: isDefaultOutput || isDefaultInput,
                        isCurrentOutput: isDefaultOutput,
                        isCurrentInput: isDefaultInput,
                        isBuiltIn: name.contains("Built-in") || name.contains("Mac") || name.contains("Internal"),
                        transport: "Unknown"
                    )
                    devices.append(device)
                }
    ...
    private func setOutputDevice(_ device: AudioDevice) {
        ...
        ' 2>/dev/null || echo "Could not set audio device"
        """)
        
        if result.success {
            alertMessage = "Output device set to \(device.name)"
        } else {
            alertMessage = "Failed to set output device: \(result.error)"
        }
        showAlert = true
        refreshAudioDevices()
    }
    ...
    private func playTestSound() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            for i in 0..<frameCount {
                audioBuffer[i] = sin(increment * Float(i)) * 0.5
            }
            
            // Convert to PCM data
            let pcmData = audioBuffer.withUnsafeBufferPointer { buffer -> Data in
                return Data(buffer: buffer)
            }
            
            // Create a temporary audio file
            let tempDir = FileManager.default.temporaryDirectory
            let audioFileURL = tempDir.appendingPathComponent("test_sound_\(Int(Date().timeIntervalSince1970)).aiff")
            
            // For simplicity, use a system beep sound instead
            // On macOS, we can use the system beep
            DispatchQueue.main.async {
                // Use NSSound for macOS audio playback
                NSSound.beep()
                
                // Also play using AVAudioPlayer as backup
    ...
    private func testMicrophone() {
        ...
        // On macOS, we can't easily record and playback like iOS
        // Instead, show device info
        alertMessage = """
        ...
        Note: On macOS, real-time microphone testing requires additional permissions and is complex to implement.
        """
        showAlert = true
    }
