...
struct SystemInfoView: View {
    ...
    private func prepareAndExport() {
        ...
        print("Export button clicked, showExportSheet: \(showExportSheet)")
    }
}
...
struct ExportSheetView: View {
    ...
    private func requestAndSaveToDesktop() {
        ...
        savePanel.begin { response in
            if response == .OK, let url = savePanel.url {
                do {
                    ...
                    self.saveAlertMessage = "✅ System information saved to Desktop:\n\(url.lastPathComponent)"
                    self.showSaveAlert = true
                    print("✅ File saved to Desktop: \(url.path)")
                    
                    // Optionally reveal the file in Finder
                    NSWorkspace.shared.selectFile(url.path, inFileViewerRootedAtPath: "")
                } catch {
                    self.saveAlertMessage = "❌ Error saving to Desktop:\n\(error.localizedDescription)"
                    self.showSaveAlert = true
    ...
    private func saveToFileWithPermission() {
        let savePanel = NSSavePanel()
