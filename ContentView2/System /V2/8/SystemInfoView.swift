...
struct SystemInfoView: View {
    ...
    private func prepareAndExport() {
        ...
        // Check if systemInfo is empty
        if systemInfo.isEmpty {
            print("System info is empty, refreshing first...")
            refreshSystemInfo()
    ...
    private func createExportContent() {
        print("Creating export content, systemInfo has \(systemInfo.count) items")
        ...
        print("First 500 chars: \(String(exportContent.prefix(500)))")
        ...
        DispatchQueue.main.async {
            ...
            print("Export sheet should show now, showExportSheet: \(self.showExportSheet)")
        }
    }
}
...
struct ExportSheetView: View {
        ...
        VStack {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 16) {
                    ...
                    .buttonStyle(.borderedProminent)
                    .disabled(exportText.isEmpty || isSaving)
    ...
    private func saveToDesktop() {
        ...
        // Use NSSavePanel to request permission
        ...
        // Force Desktop as the target directory
        ...
        // Show the save panel and handle the response
        savePanel.begin { response in
            if response == .OK, let url = savePanel.url {
                self.saveToURL(url, isDesktopSave: true)
            } else if response == .cancel {
                // User cancelled
                self.saveAlertMessage = "Save cancelled by user"
    ...
    private func saveAs() {
        ...
        savePanel.begin { response in
            if response == .OK, let url = savePanel.url {
                self.saveToURL(url, isDesktopSave: false)
            }
        }
    }
    
    private func saveToURL(_ url: URL, isDesktopSave: Bool = false) {
        do {
            try exportText.write(to: url, atomically: true, encoding: .utf8)
            let location = isDesktopSave ? "Desktop" : "location"
            saveAlertMessage = "✅ System information saved to \(location):\n\(url.lastPathComponent)"
            showSaveAlert = true
            print("✅ File saved to: \(url.path)")
            
            // Optionally reveal the file in Finder
