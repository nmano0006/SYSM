...
struct SystemInfoView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                // NEW: Bootloader Information Section
    ...
    private var BootloaderInfoSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            SectionHeader(title: "Bootloader Information", icon: "bolt.fill")
            
            VStack(alignment: .leading, spacing: 12) {
                InfoRow(label: "Bootloader Type:", value: systemInfo["bootloaderType"] ?? "Unknown")
                InfoRow(label: "Bootloader Version:", value: systemInfo["bootloaderVersion"] ?? "Unknown")
                InfoRow(label: "EFI Firmware:", value: systemInfo["efiFirmware"] ?? "Unknown")
                InfoRow(label: "Boot Mode:", value: systemInfo["bootMode"] ?? "Unknown")
                InfoRow(label: "Boot Args:", value: systemInfo["bootArgs"] ?? "Unknown")
                InfoRow(label: "Secure Boot:", value: systemInfo["secureBootStatus"] ?? "Unknown")
                InfoRow(label: "VMM Detection:", value: systemInfo["vmmDetection"] ?? "Unknown")
            }
            .padding()
            .background(Color(.controlBackgroundColor))
            .cornerRadius(12)
    ...
    private func generateExportText() -> String {
        ...
        text += "========================================\n"
        text += "Bootloader Type: \(systemInfo["bootloaderType"] ?? "Unknown")\n"
        text += "Bootloader Version: \(systemInfo["bootloaderVersion"] ?? "Unknown")\n"
        text += "EFI Firmware: \(systemInfo["efiFirmware"] ?? "Unknown")\n"
        text += "Boot Mode: \(systemInfo["bootMode"] ?? "Unknown")\n"
        text += "Boot Args: \(systemInfo["bootArgs"] ?? "Unknown")\n"
        text += "Secure Boot: \(systemInfo["secureBootStatus"] ?? "Unknown")\n"
        text += "VMM Detection: \(systemInfo["vmmDetection"] ?? "Unknown")\n"
    ...
    private func exportSystemInfo() {
        ...
        // Create a temporary file to show in the save panel
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("system-info-\(formattedDate()).txt")
        ...
        do {
            ...
            savePanel.nameFieldStringValue = "system-info-\(formattedDate()).txt"
            ...
            savePanel.begin { response in
                if response == .OK, let url = savePanel.url {
                    do {
                        ...
                        print("Export successful to: \(url.path)")
                    } catch {
                        self.exportErrorMessage = "Failed to save file: \(error.localizedDescription)"
            ...
            isExporting = false
            exportErrorMessage = "Failed to prepare export: \(error.localizedDescription)"
            showExportError = true
            print("Export preparation failed: \(error.localizedDescription)")
        }
    }
    ...
    private func refreshSystemInfo() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            // NEW: Get bootloader information
            let bootloaderType = ShellHelper.runCommand("nvram 4D1FDA02-38C7-4A6A-9CC6-4BCCA8B30102:opencore-version 2>/dev/null || echo 'Unknown'").output
            if !bootloaderType.isEmpty && !bootloaderType.contains("Unknown") {
                info["bootloaderType"] = "OpenCore"
                info["bootloaderVersion"] = bootloaderType.trimmingCharacters(in: .whitespacesAndNewlines)
            } else {
                let cloverVersion = ShellHelper.runCommand("nvram 4D1FDA02-38C7-4A6A-9CC6-4BCCA8B30102:clover-version 2>/dev/null || echo 'Unknown'").output
                if !cloverVersion.isEmpty && !cloverVersion.contains("Unknown") {
                    info["bootloaderType"] = "Clover"
                    info["bootloaderVersion"] = cloverVersion.trimmingCharacters(in: .whitespacesAndNewlines)
                } else {
                    info["bootloaderType"] = "Native (Apple)"
                    info["bootloaderVersion"] = "Apple EFI"
                }
            }
            
            // Get EFI firmware info
            let efiInfo = ShellHelper.runCommand("system_profiler SPHardwareDataType | grep 'Boot ROM Version:' | awk -F': ' '{print $2}'").output
            info["efiFirmware"] = efiInfo.isEmpty ? "Unknown" : efiInfo.trimmingCharacters(in: .whitespacesAndNewlines)
            
            // Get boot mode
            let bootMode = ShellHelper.runCommand("nvram boot-args 2>/dev/null | awk '{print $2}'").output
            info["bootMode"] = bootMode.isEmpty ? "Normal" : bootMode.trimmingCharacters(in: .whitespacesAndNewlines)
            
            // Get boot args
            let bootArgs = ShellHelper.runCommand("nvram boot-args 2>/dev/null").output
            info["bootArgs"] = bootArgs.isEmpty ? "None" : bootArgs.trimmingCharacters(in: .whitespacesAndNewlines)
            
            // Get secure boot status
            let secureBoot = ShellHelper.runCommand("nvram apple-secure-boot 2>/dev/null").output
            info["secureBootStatus"] = secureBoot.isEmpty ? "Disabled" : "Enabled"
            
            // VMM detection (for Hackintosh/Virtual Machine)
            let vmmDetection = ShellHelper.runCommand("sysctl -n kern.hv_vmm_present 2>/dev/null").output
            info["vmmDetection"] = vmmDetection.contains("1") ? "Virtual Machine Detected" : "Native Hardware"
...
// Add the ExportSuccessView struct
struct ExportSuccessView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text("System information has been exported to:")
                .font(.body)
                .foregroundColor(.secondary)
