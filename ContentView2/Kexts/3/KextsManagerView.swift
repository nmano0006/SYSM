...
struct KextsManagerView: View {
    ...
    @State private var selectedTab = 0  // 0 = Manage Kexts, 1 = Install AppleHDA
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                HStack {
                    Text(selectedTab == 0 ? "Kernel Extensions" : "AppleHDA Installer")
                        .font(.title)
                        .fontWeight(.bold)
    ...
    private func StatusItem(title: String, status: String, version: String?, icon: String, color: Color) -> some View {
        VStack(spacing: 8) {
            ...
            Text(status)
                .font(.caption)
                .foregroundColor(status == "Installed" || status == "Loaded" ? .green : .orange)
                .padding(.horizontal, 8)
                .padding(.vertical, 2)
                .background(
                    (status == "Installed" || status == "Loaded" ? Color.green : Color.orange)
                        .opacity(0.1)
                )
                .cornerRadius(4)
