...
struct KextsManagerView: View {
    ...
    private func loadKext(_ kext: KextInfo) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let result = shellHelper.runCommand("sudo kextload \"\(kext.path)\"", needsSudo: true)
            
            DispatchQueue.main.async {
                if result.success {
                    alertMessage = "Successfully loaded \(kext.name)"
                } else {
                    alertMessage = "Failed to load \(kext.name): \(result.error)"
                }
                showInfoAlert = true
                operationInProgress = false
                refreshKexts()
    ...
    private func unloadKext(_ kext: KextInfo) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let shellHelper = ShellHelper.shared
            let result = shellHelper.runCommand("sudo kextunload -b \(kext.bundleID)", needsSudo: true)
            
            DispatchQueue.main.async {
                if result.success {
                    alertMessage = "Successfully unloaded \(kext.name)"
                } else {
                    alertMessage = "Failed to unload \(kext.name): \(result.error)"
                }
                showInfoAlert = true
                operationInProgress = false
                refreshKexts()
