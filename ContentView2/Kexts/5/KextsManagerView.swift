...
struct KextsManagerView: View {
    ...
    private func installAudioPackage() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            _ = self.runSudoCommand("mkdir -p \(ocKextsPath)")
            _ = self.runSudoCommand("mkdir -p /System/Library/Extensions")
            ...
            // Disable System Integrity Protection temporarily for /S/L/E installation
            messages.append("\n⚠️ Disabling System Integrity Protection temporarily...")
            let csrutilResult = self.runSudoCommand("csrutil disable")
            if csrutilResult.success {
                messages.append("✅ SIP disabled")
            } else {
                messages.append("⚠️ Could not disable SIP (may already be disabled)")
            }
            ...
            if isDirectory.boolValue {
                    ...
                    if let sourcePath = self.findKextInDirectory(name: name, directory: self.kextSourcePath) {
                        ...
                        if destination == "/S/L/E" {
                            let commands = [
                                ...
                                "cp -R \"\(sourcePath)\" \"/System/Library/Extensions/\(name).kext\"",
                                "chown -R root:wheel \"/System/Library/Extensions/\(name).kext\"",
                                "chmod -R 755 \"/System/Library/Extensions/\(name).kext\"",
                                "touch /System/Library/Extensions"
                            ]
                            
                            for cmd in commands {
                                let result = self.runSudoCommand(cmd)
                            ...
                            let command = "cp -R \"\(sourcePath)\" \"\(ocKextsPath)\(name).kext\""
                            let result = self.runSudoCommand(command)
                            if !result.success {
                            ...
                            _ = self.runSudoCommand("chmod -R 755 \"\(ocKextsPath)\(name).kext\"")
                            _ = self.runSudoCommand("chown -R root:wheel \"\(ocKextsPath)\(name).kext\"")
                        }
                ...
                // Install Lilu to EFI
                _ = findAndInstallKext(name: "Lilu", destination: "EFI")
                
                // Install AppleALC to EFI
                _ = findAndInstallKext(name: "AppleALC", destination: "EFI")
                ...
                if appleHDAInstalled {
                    ...
                    _ = self.runSudoCommand("nvram boot-args=\"-v keepsyms=1 debug=0x100 alcid=1\"")
                }
                ...
                if self.kextSourcePath.hasSuffix(".kext") {
                    ...
                    if kextName.lowercased() == "applehda" {
                        // Install AppleHDA to /S/L/E
                        let commands = [
                            "rm -rf \"/System/Library/Extensions/AppleHDA.kext\"",
                            "cp -R \"\(self.kextSourcePath)\" \"/System/Library/Extensions/AppleHDA.kext\"",
                        ...
                        let command = "cp -R \"\(self.kextSourcePath)\" \"\(ocKextsPath)\(fileName)\""
                        let result = self.runSudoCommand(command)
                        if result.success {
                            messages.append("✅ \(kextName).kext installed to EFI")
                            _ = self.runSudoCommand("chmod -R 755 \"\(ocKextsPath)\(fileName)\"")
                            _ = self.runSudoCommand("chown -R root:wheel \"\(ocKextsPath)\(fileName)\"")
                        } else {
            ...
            _ = self.runSudoCommand("csrutil enable")
            messages.append("✅ SIP re-enabled")
            
            // Rebuild kernel cache
            if success {
                messages.append("\n🔄 Rebuilding kernel cache...")
                let result = self.runSudoCommand("kextcache -i /")
                if result.success {
    ...
    private func loadKext(_ kext: KextInfo) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let result = self.runSudoCommand("kextload \"\(kext.path)\"")
    ...
    private func unloadKext(_ kext: KextInfo) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let result = self.runSudoCommand("kextunload -b \(kext.bundleID)")
    ...
    // MARK: - Helper Methods
    private func runSudoCommand(_ command: String) -> (output: String, success: Bool) {
        return ShellHelper.runSudoCommand(command)
    }
}