...
struct KextsManagerView: View {
            ...
            if let efiPath = efiPath {
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        ...
                        Spacer()
                        Button("Unmount") {
                            ShellHelper.runCommand("diskutil unmount \(efiPath)")
                            self.efiPath = nil
                        }
                        .font(.caption)
                    }
    ...
    private func verifyKDKInstallation() {
        ...
        if totalKDKs > 0 {
            ...
            _ = ShellHelper.runCommand("ls -d \"/Library/Developer/KDKs/KDK_26.2_25C56.kdk\" 2>/dev/null")
    ...
    private func installAudioPackage() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            _ = self.runSudoCommandDirect("mkdir -p \(ocKextsPath)")
            _ = self.runSudoCommandDirect("mkdir -p /System/Library/Extensions")
            ...
            // Check SIP status - don't try to disable if already disabled
            messages.append("\n🔒 Checking System Integrity Protection...")
            let csrutilCheck = ShellHelper.runCommand("csrutil status")
            let sipEnabled = csrutilCheck.output.contains("enabled")
            
            if sipEnabled {
                messages.append("⚠️ SIP is enabled - some operations may be restricted")
                messages.append("For best results, disable SIP in Recovery Mode: csrutil disable")
            } else {
                messages.append("✅ SIP is already disabled")
            }
            ...
            if isDirectory.boolValue {
                ...
                // Helper function to find kext
                func findAndInstallKext(name: String, destination: String) -> Bool {
                    if let sourcePath = self.findKextInDirectory(name: name, directory: self.kextSourcePath) {
                        messages.append("\n📦 Installing \(name).kext...")
                        
                        if destination == "/S/L/E" {
                            // Use simple sudo command instead of AppleScript for file operations
                            let commands = [
                                "sudo rm -rf \"/System/Library/Extensions/\(name).kext\"",
                                "sudo cp -R \"\(sourcePath)\" \"/System/Library/Extensions/\(name).kext\"",
                            ...
                            for cmd in commands {
                                // Use direct sudo command (cached credentials)
                            ...
                            let command = "sudo cp -R \"\(sourcePath)\" \"\(ocKextsPath)\(name).kext\""
                            let result = ShellHelper.runCommand(command)
                            if !result.success {
                                messages.append("❌ Failed to install \(name).kext to EFI")
                                messages.append("Error: \(result.output)")
                                return false
                            }
                            
                            // Set permissions for EFI kexts
                            _ = ShellHelper.runCommand("sudo chmod -R 755 \"\(ocKextsPath)\(name).kext\"")
                            _ = ShellHelper.runCommand("sudo chown -R root:wheel \"\(ocKextsPath)\(name).kext\"")
                        }
                        messages.append("✅ \(name).kext installed successfully")
                        return true
                    } else {
                        messages.append("⚠️ \(name).kext not found in source directory")
                        return false
                    }
                }
                
                // Install Lilu to EFI
                let liluInstalled = findAndInstallKext(name: "Lilu", destination: "EFI")
                
                // Install AppleALC to EFI
                let appleALCInstalled = findAndInstallKext(name: "AppleALC", destination: "EFI")
                
                // Install AppleHDA to /System/Library/Extensions
                let appleHDAInstalled = findAndInstallKext(name: "AppleHDA", destination: "/S/L/E")
                
                // Create boot arguments if needed
                if appleHDAInstalled {
                    messages.append("\n🔧 Setting boot arguments for AppleHDA...")
                    _ = ShellHelper.runCommand("sudo nvram boot-args=\"-v keepsyms=1 debug=0x100 alcid=1\"")
                }
                
            } else {
                // Install from single kext file
                if self.kextSourcePath.hasSuffix(".kext") {
                    let fileName = URL(fileURLWithPath: self.kextSourcePath).lastPathComponent
                    let kextName = fileName.replacingOccurrences(of: ".kext", with: "")
                    messages.append("\n📦 Installing \(kextName).kext...")
                    
                    if kextName.lowercased() == "applehda" {
                        // Install AppleHDA to /S/L/E using direct sudo commands
                        let commands = [
                            "sudo rm -rf \"/System/Library/Extensions/AppleHDA.kext\"",
                            "sudo cp -R \"\(self.kextSourcePath)\" \"/System/Library/Extensions/AppleHDA.kext\"",
                        ...
                        let command = "sudo cp -R \"\(self.kextSourcePath)\" \"\(ocKextsPath)\(fileName)\""
                        let result = ShellHelper.runCommand(command)
                        if result.success {
                            messages.append("✅ \(kextName).kext installed to EFI")
                            _ = ShellHelper.runCommand("sudo chmod -R 755 \"\(ocKextsPath)\(fileName)\"")
                            _ = ShellHelper.runCommand("sudo chown -R root:wheel \"\(ocKextsPath)\(fileName)\"")
                        } else {
            ...
            if success {
                ...
                // Use direct sudo command for kernel cache rebuild
                let result = ShellHelper.runCommand("sudo kextcache -i /")
                if result.success {
                    messages.append("✅ Kernel cache rebuilt successfully")
                } else {
                    messages.append("⚠️ Kernel cache rebuild returned warnings:")
                    messages.append(result.output)
                    
                    // Try alternative method
                    messages.append("\n🔄 Trying alternative cache rebuild method...")
                    let altResult = ShellHelper.runCommand("sudo kmutil install --update-all")
                    if altResult.success {
                        messages.append("✅ Kernel cache rebuilt using alternative method")
                    }
                }
            }
            ...
            if success {
                ...
                messages.append("✅ Audio package installed successfully")
                messages.append("✅ Permissions set correctly")
                messages.append("✅ Kernel cache rebuilt")
                messages.append("\n🔄 REQUIRED: Restart your computer for changes to take effect")
            } else {
                messages.append("\n❌ Installation failed or incomplete")
                messages.append("⚠️ Some kexts may not have installed correctly")
                messages.append("⚠️ You may need to manually disable SIP in Recovery Mode")
                messages.append("⚠️ Check sudo password or use Terminal for manual installation")
            }
    ...
    private func findKextInDirectory(name: String, directory: String) -> String? {
        ...
        do {
            ...
            for item in contents {
                ...
                if fileManager.fileExists(atPath: itemPath, isDirectory: &isDir) {
                    if isDir.boolValue && item.lowercased().contains(name.lowercased()) && (item.lowercased().hasSuffix(".kext") || item.lowercased().hasSuffix(".plugin")) {
                        return itemPath
                    }
                }
            }
            ...
            for item in contents {
                ...
                if fileManager.fileExists(atPath: itemPath, isDirectory: &isDir) && isDir.boolValue {
                    ...
                    if item.lowercased().contains("system") || item.lowercased().contains("library") {
                        continue
                    }
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                ...
                if result.success {
                    ...
                    self.alertMessage = "Failed to rebuild cache:\n\(result.output)\n\nTry running in Terminal: sudo kextcache -i /"
                }
                self.showInfoAlert = true
            }
        }
    ...
    // MARK: - Helper Methods
    private func runSudoCommandDirect(_ command: String) -> (output: String, success: Bool) {
        // Use direct sudo command (cached credentials)
        return ShellHelper.runCommand("sudo \(command)")
    }
}