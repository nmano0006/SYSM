            ...
            HStack(alignment: .center) {
                ...
                if isCurrentlyEditing && entry.type != "Dictionary" && entry.type != "Array" {
                    TextField("Value", text: Binding(
                        get: { entry.editedValue },
                        set: { newValue in
                            if let index = entry.editedValue.firstIndex(of: newValue) {
                                // Update the entry's editedValue
                                var updatedEntry = entry
                                updatedEntry.editedValue = newValue
                    ...
                    HStack(spacing: 4) {
                        if isCurrentlyEditing {
                            Button(action: {
                                ...
                                // We need a way to update this in the parent
                                onSaveEditing()
                            }) {
