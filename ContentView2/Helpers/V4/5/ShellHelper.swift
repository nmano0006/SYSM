...
struct ShellHelper {
    ...
    static func detectOpenCore() -> OpenCoreInfo? {
        ...
        // Determine if this is a Hackintosh
        var isHackintosh = false
        if bootloader.name.contains("Hackintosh") {
            isHackintosh = true
        } else {
            // Check for non-Apple hardware indicators
    ...
    static func getOpenCoreConfig() -> [String: Any]? {
        ...
        // First, try to mount EFI if not already mounted
        ...
        // If we mounted EFI just for reading, unmount it
        ...
        if foundConfig == nil {
            ...
            // Try to find config in user's documents or downloads
