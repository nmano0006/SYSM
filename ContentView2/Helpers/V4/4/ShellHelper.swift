...
struct ShellHelper {
    ...
    // MARK: - Get Bootloader Details (for SystemInfoView compatibility)
    static func getBootloaderDetails() -> [String: String] {
        ...
        guard let openCoreInfo = detectOpenCore() else {
            ...
            details["sipStatus"] = "Unknown"
            details["currentBootVolume"] = "Unknown"
        ...
        // For OpenCore, try to get more details
        if openCoreInfo.name.contains("OpenCore") {
            // Check if using custom SSDT/ACPI
            if let efiPath = openCoreInfo.efiMountPath {
                let acpiCheck = runCommand("""
                if [ -d \(efiPath)/EFI/OC/ACPI ]; then
                    ls \(efiPath)/EFI/OC/ACPI/*.aml 2>/dev/null | wc -l | tr -d ' '
                else
                    echo "0"
                fi
                """)
                ...
                if [ -d \(efiPath)/EFI/OC/Kexts ]; then
                    ls \(efiPath)/EFI/OC/Kexts/*.kext 2>/dev/null | wc -l | tr -d ' '
                else
                    echo "0"
                fi
                """)
