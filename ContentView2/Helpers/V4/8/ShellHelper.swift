// ShellHelper.swift (Updated with EFI mounting functions)
import Foundation
import AppKit
...
struct ShellHelper {
    ...
    static func mountEFIDrive(_ identifier: String) -> (success: Bool, message: String) {
        ...
        // Try to mount using standard method first
        let mountResult = runCommand("diskutil mount /dev/\(identifier)")
        
        if mountResult.success {
            return (true, "✅ Successfully mounted EFI partition")
        } else {
            // If standard mount fails, try creating mount point and using mount command
            ...
            _ = runCommand("sudo mkdir -p \(mountDir) 2>/dev/null || true")
            
            // Mount using mount command with correct filesystem type
            let altResult = runCommand("sudo mount -t msdos /dev/\(identifier) \(mountDir)")
            
            if altResult.success {
                return (true, "✅ Successfully mounted EFI partition to \(mountDir)")
            } else {
                // Try with hfs filesystem type (for older systems)
                let hfsResult = runCommand("sudo mount -t hfs /dev/\(identifier) \(mountDir)")
                
                if hfsResult.success {
                    return (true, "✅ Successfully mounted EFI partition (HFS) to \(mountDir)")
                } else {
                    // Last resort: try diskutil mountDisk
                    let diskResult = runCommand("diskutil mountDisk /dev/\(identifier)")
                    
                    if diskResult.success {
                        return (true, "✅ Successfully mounted EFI partition using mountDisk")
                    } else {
                        return (false, "❌ Failed to mount EFI partition. Error: \(mountResult.output)")
                    }
                }
            }
        }
    ...
    // MARK: - Existing Functions (Updated for EFI)
    ...
    static func getAllDrives() -> [DriveInfo] {
        ...
        // Also get EFI partitions
        let efiPartitions = getEFIPartitionsList()
        print("📌 Found \(efiPartitions.count) EFI partitions")
        drives.append(contentsOf: efiPartitions)
        ...
        uniqueDrives.sort {
            // EFI partitions first if not mounted
            if $0.isEFI != $1.isEFI {
                return $0.isEFI && !$0.isMounted
            }
            // Mounted drives first
            if $0.isMounted != $1.isMounted {
                return $0.isMounted && !$1.isMounted
            }
            // Internal drives last
    ...
    private static func getMountedVolumes() -> [DriveInfo] {
        ...
        for line in dfLines {
            ...
            if devicePath.hasPrefix("/dev/disk") {
                ...
                // Include EFI partitions in mounted volumes
                if deviceId.contains("EFI") || mountPoint == "/Volumes/EFI" {
                    // Don't skip EFI - include them
                }
            ...
            if line.contains("disk") && line.contains("s") {
                ...
                if let partitionId = components.first(where: { $0.hasPrefix("disk") && $0.contains("s") }) {
                    if !currentDisk.isEmpty && partitionId.hasPrefix(currentDisk) {
                        ...
                        if !isMounted {
                            // Filter out system recovery partitions but keep EFI
    ...
    private static func getEFIPartitionsList() -> [DriveInfo] {
        print("📌 Getting EFI partitions list...")
        ...
        let efiIds = result.output.components(separatedBy: "\n").filter { !$0.isEmpty }
        
        for efiId in efiIds {
            let info = getEFIDriveInfo(efiId)
            ...
            let efiDrive = DriveInfo(
                ...
                isInternal: !efiId.contains("disk2") && !efiId.contains("disk3") && !efiId.contains("disk4"),
                isEFI: true,
                partitions: [],
    ...
    private static func getDriveInfo(deviceId: String) -> DriveInfo {
        ...
        // Check for EFI by identifier pattern
        if !isEFI && (deviceId.contains("EFI") || deviceId.contains("s1") || deviceId.contains("s2")) {
            // Check if it's likely an EFI partition by size
            if size.contains("MB") || size.contains("KB") || size == "209.7 MB" || size == "314.6 MB" {
                let checkEFI = runCommand("diskutil info /dev/\(deviceId) | grep 'EFI'")
                if checkEFI.success {
                    isEFI = true
                    type = "EFI"
                    name = "EFI System Partition"
    ...
    static func mountSelectedDrives(drives: [DriveInfo]) -> (success: Bool, message: String) {
        ...
        for drive in drives where drive.isSelectedForMount && !drive.isMounted {
            ...
            if drive.isEFI {
                let efiResult = mountEFIDrive(drive.identifier)
                if efiResult.success {
                    successCount += 1
                    messages.append("✅ \(drive.name): EFI partition mounted")
                } else {
                    failedCount += 1
                    messages.append("❌ \(drive.name): Failed to mount EFI - \(efiResult.message)")
                }
                continue
            }
    ...
    static func unmountSelectedDrives(drives: [DriveInfo]) -> (success: Bool, message: String) {
        ...
        for drive in drives where drive.isSelectedForUnmount && drive.isMounted {
            ...
            // Special handling for EFI partitions
            if drive.isEFI {
                let efiResult = unmountEFIDrive(drive.identifier)
                if efiResult.success {
                    successCount += 1
                    messages.append("✅ \(drive.name): EFI partition unmounted")
                } else {
                    failedCount += 1
