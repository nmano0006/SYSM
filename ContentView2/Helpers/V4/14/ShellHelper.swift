...
struct ShellHelper {
    ...
    static func getAllDrives() -> [DriveInfo] {
        ...
        // Get complete disk list
        let diskListResult = runCommand("diskutil list")
        print("📊 Raw diskutil list output length: \(diskListResult.output.count) characters")
        
        // Parse diskutil list output to find all partitions
        let lines = diskListResult.output.components(separatedBy: "\n")
        var currentDisk = ""
        var partitionLines: [(identifier: String, size: String, type: String)] = []
        
        for line in lines {
            // Check for disk header (e.g., "/dev/disk4 (internal, physical):")
            if line.contains("/dev/disk") && line.contains("disk") && (line.contains("(internal") || line.contains("(external")) {
                currentDisk = line.components(separatedBy: " ")[0].replacingOccurrences(of: "/dev/", with: "")
            }
            
            // Parse partition lines (they contain identifiers like disk4s1, disk4s2, etc.)
            if line.contains(currentDisk) && line.contains("s") && line.contains("B") {
                let components = line.components(separatedBy: .whitespaces).filter { !$0.isEmpty }
                
                // Look for partition identifier (e.g., disk4s1)
                for component in components {
                    if component.starts(with: currentDisk) && component.contains("s") {
                        let identifier = component
                        
                        // Extract size (usually before partition type)
                        var size = "Unknown"
                        var type = "Unknown"
                        var isEFI = false
                        
                        // Check for EFI indicators
                        if line.contains("EFI") || line.contains("Microsoft Basic Data") {
                            type = "EFI"
                            isEFI = true
                            // EFI partitions are typically 200MB, 500MB, or 1GB
                            if line.contains("200") || line.contains("209") {
                                size = "209.7 MB"
                            } else if line.contains("500") || line.contains("524") {
                                size = "500.0 MB"
                            } else if line.contains("1.0") || line.contains("1024") {
                                size = "1.0 GB"
                            } else {
                                // Try to extract size from line
                                for comp in components {
                                    if comp.contains("B") && (comp.contains("M") || comp.contains("G") || comp.contains("K")) {
                                        size = comp
                                        break
                                    }
                                }
                            ...
                            // Extract size for non-EFI partitions
                            for comp in components {
                                if comp.contains("B") && (comp.contains("M") || comp.contains("G") || comp.contains("K")) {
                                    size = comp
                                    break
                            ...
                            if line.contains("APFS") {
                                type = "APFS"
                            } else if line.contains("HFS+") || line.contains("Mac OS Extended") {
                                type = "HFS+"
                            } else if line.contains("ExFAT") {
                                type = "ExFAT"
                            } else if line.contains("NTFS") {
                                type = "NTFS"
                            } else if line.contains("FAT32") {
                                type = "FAT32"
                            }
                        }
                        
                        if !identifier.isEmpty && identifier != currentDisk {
                            partitionLines.append((identifier: identifier, size: size, type: type))
                            
                            // Get detailed info for each partition
                            let detailedInfo = getDriveInfo(deviceId: identifier)
                            var driveInfo = detailedInfo
                            
                            // Override with parsed EFI info
                            if isEFI {
                                driveInfo = DriveInfo(
                                    ...
                                    isInternal: detailedInfo.isInternal,
                                    isEFI: true,
                                    partitions: [],
                                    isMounted: detailedInfo.isMounted,
                                    isSelectedForMount: false,
                                    isSelectedForUnmount: false
                                )
                            }
                            
                            // Add to drives list if not already present
        ...
        // Also get mounted volumes from df command
        ...
        for line in dfLines {
            ...
            if parts.count >= 6 && parts[0].hasPrefix("/dev/disk") {
                ...
                if let existingIndex = drives.firstIndex(where: { $0.identifier == deviceId }) {
                    // Update existing drive with mount info
                    var existingDrive = drives[existingIndex]
                    let updatedDrive = DriveInfo(
                        name: existingDrive.name,
                        identifier: existingDrive.identifier,
                        size: existingDrive.size,
                        type: existingDrive.type,
                        mountPoint: mountPoint,
                        isInternal: existingDrive.isInternal,
                        isEFI: existingDrive.isEFI,
                        partitions: existingDrive.partitions,
                        isMounted: true,
                        isSelectedForMount: existingDrive.isSelectedForMount,
                        isSelectedForUnmount: existingDrive.isSelectedForUnmount
                    )
                    drives[existingIndex] = updatedDrive
                } else {
                    // Get detailed info for new mounted drive
        ...
        // Get ALL disk partitions using diskutil list - more comprehensive method
        let allPartitionsResult = runCommand("""
        diskutil list | grep -oE 'disk[0-9]+s[0-9]+' | sort -u
        """)
        
        let allPartitionIds = allPartitionsResult.output.components(separatedBy: "\n").filter { !$0.isEmpty }
        print("🔍 Found \(allPartitionIds.count) total partitions")
        ...
        for partitionId in allPartitionIds {
            ...
            // Check if it might be an EFI partition by size
            var isPossibleEFI = false
            if (drive.size.contains("209.7 MB") || 
                drive.size.contains("200.0 MB") || 
                drive.size.contains("500.0 MB") || 
                drive.size.contains("1.0 GB")) && 
               !drive.isMounted {
                isPossibleEFI = true
                
                // Double-check with diskutil info
                let typeCheck = runCommand("diskutil info /dev/\(partitionId) | grep -i 'type.*efi'")
            ...
            // Only add if it's not already in the list and not a duplicate
            if !drives.contains(where: { $0.identifier == partitionId }) {
                drives.append(drive)
            }
        }
        ...
        for drive in sortedDrives {
            print("   - \(drive.identifier): \(drive.name) (\(drive.type)) \(drive.isMounted ? "Mounted at \(drive.mountPoint)" : "Unmounted") \(drive.isEFI ? "EFI" : "")")
        }
    ...
    static func getDriveInfo(deviceId: String) -> DriveInfo {
            ...
            if trimmed.contains("Volume Name:") {
                ...
                if parts.count > 1 {
                    ...
                    if protocolType.contains("USB") || protocolType.contains("External") || protocolType.contains("SAS") {
                        ...
                        isInternal = true
                        type = "NVMe"
                    } else if protocolType.contains("Disk Image") {
                        isInternal = false
                        type = "Disk Image"
                    }
                }
        ...
        // Detect external drives by disk number (disk0, disk1 are usually internal on Apple Silicon)
        if !deviceId.starts(with: "disk0") && !deviceId.starts(with: "disk1") && !isEFI {
            // Double-check with diskutil info
            let internalCheck = runCommand("diskutil info /dev/\(deviceId) | grep -i 'internal:'")
            if internalCheck.output.contains("No") || internalCheck.output.contains("false") {
                isInternal = false
            }
