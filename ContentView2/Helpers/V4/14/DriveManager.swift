...
class DriveManager: ObservableObject {
    ...
    func testEFIDetection() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            echo "=== FULL DISKUTIL LIST ==="
            diskutil list
            echo ""
            echo "=== ALL DISK IDENTIFIERS ==="
            diskutil list | grep -oE '/dev/disk[0-9]+s[0-9]+' | sort -u
            echo ""
            echo "=== ALL PARTITION TYPES ==="
            for disk in $(diskutil list | grep -oE 'disk[0-9]+s[0-9]+'); do
                echo "Checking $disk:"
                diskutil info /dev/$disk | grep -E '(Type|Volume Name|Disk Size):' | head -3
            done
            echo ""
