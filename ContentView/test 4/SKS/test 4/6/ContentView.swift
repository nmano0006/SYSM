...
class ShellHelper {
    ...
    func performEFICheck() -> String {
        ...
        let systemInfo = runCommand("system_profiler SPSoftwareDataType | grep 'System Version:'")
        messages.append(systemInfo.output.isEmpty ? "macOS Version: Unknown" : systemInfo.output)
        
        // Check disk utility status
        messages.append("\n💾 Disk Utility Status:")
        let diskUtilStatus = runCommand("diskutil list | head -20")
        if diskUtilStatus.success {
            messages.append("Disk Utility is responsive")
        } else {
            messages.append("⚠️ Disk Utility may not be responding")
        }
        
        // Check for EFI partitions
        messages.append("\n🔐 EFI Partition Scan:")
        let efiResult = runCommand("diskutil list | grep -A2 -B2 -i 'EFI\\|Microsoft Basic Data'")
        
        if efiResult.success && !efiResult.output.isEmpty {
            messages.append("✅ EFI partitions detected:")
            let lines = efiResult.output.components(separatedBy: "\n")
            for line in lines {
                if line.contains("disk") {
                    messages.append("   " + line.trimmingCharacters(in: .whitespaces))
                }
            }
        } else {
            ...
            // Try alternative method
            messages.append("\n🔍 Alternative EFI detection:")
            let altCheck = runCommand("ls /dev/disk*s1 2>/dev/null")
            if !altCheck.output.isEmpty {
                let partitions = altCheck.output.components(separatedBy: "\n").filter { !$0.isEmpty }
                messages.append("Found potential EFI partitions (first partitions):")
                for partition in partitions {
                    messages.append("   " + partition.replacingOccurrences(of: "/dev/", with: ""))
                }
            }
        }
        
        // Check if /Volumes/EFI exists
        ...
        // Check current EFI mounts
        messages.append("\n📌 Current EFI Mount Status:")
        let mountCheck = runCommand("mount | grep -i 'efi\\|/dev/disk.*s1' || echo 'No EFI partitions currently mounted'")
        messages.append(mountCheck.output)
        ...
        if sudoCheck.success {
            messages.append("✅ Sudo access available")
        } else {
            messages.append("⚠️ Sudo access may require password")
        }
        
        // Check disk space
        messages.append("\n💾 Available Disk Space:")
        let diskSpace = runCommand("df -h / /Volumes 2>/dev/null | head -5")
        if diskSpace.success {
            let lines = diskSpace.output.components(separatedBy: "\n")
            for line in lines {
                if !line.isEmpty {
                    messages.append("   " + line)
                }
        ...
        // Check common EFI partitions specifically
        messages.append("\n🔍 Specific EFI Partition Status:")
        let commonEFIs = ["disk0s1", "disk1s1", "disk2s1", "disk3s1", "disk4s1"]
        
        for efi in commonEFIs {
            let check = runCommand("diskutil info /dev/\(efi) 2>/dev/null | grep -E 'Volume Name|Type|Mount Point' | head -3")
        ...
        // Recommendations
        messages.append("\n💡 Recommendations:")
        if mountCheck.output.contains("No EFI partitions") {
            messages.append("1. Try mounting disk1s1 or disk2s1 (your macOS drives)")
            messages.append("2. Ensure you have administrator privileges")
            messages.append("3. Check if diskutil is working: run 'diskutil list' in Terminal")
        } else {
            messages.append("1. EFI partitions are accessible")
            messages.append("2. You can mount/unmount as needed")
    ...
    func performEFICheck() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                ...
                self.isLoading = false
                self.refreshDrives() // Refresh the drive list after check
            }
        }
    }
