...
// MARK: - Shell Helper (Fixed with improved debugging)
class ShellHelper {
    ...
    func findUSBDrives() -> [String] {
        ...
        diskutil list | grep -oE 'disk[0-9]+' | while read disk; do
            if diskutil info /dev/$disk 2>/dev/null | grep -E 'Protocol.*USB|Bus Protocol.*USB|Removable.*Yes' >/dev/null; then
                echo $disk
            fi
        done
        """)
        ...
        // Method 2: Using system_profiler as fallback
        if usbDrives.isEmpty {
            print("🔄 Trying alternative USB detection method...")
            let profilerResult = runCommand("""
            system_profiler SPUSBDataType 2>/dev/null | grep -A 5 -B 5 "Mount Point" | grep "/Volumes" | sed 's/.*\\/Volumes\\///' | head -5
            """)
            
            if !profilerResult.output.isEmpty {
                print("📊 Alternative detection found: \(profilerResult.output)")
            }
        }
    ...
    func getAllDrives() -> [DriveInfo] {
        ...
        // Try multiple methods to get disk information
        var listResult = runCommand("diskutil list")
        
        // If diskutil fails, try alternative command
        if !listResult.success || listResult.output.isEmpty {
            print("⚠️ diskutil list failed, trying alternative...")
            listResult = runCommand("ls /dev/disk* 2>&1")
            print("Alternative output: \(listResult.output)")
        }
        ...
        if listResult.success && !listResult.output.isEmpty {
            ...
            print("❌ Could not get disk list. Error: \(listResult.error)")
        }
        
        // Fallback: If no drives found, try system_profiler
        if drives.isEmpty {
            print("🔄 Trying system_profiler fallback...")
            drives = getDrivesFromSystemProfiler()
        }
        
        // Get mount info for each drive
        print("📌 Getting mount info for \(drives.count) drives...")
    ...
    private func getDrivesFromSystemProfiler() -> [DriveInfo] {
        var drives: [DriveInfo] = []
        
        let profilerResult = runCommand("""
        system_profiler SPStorageDataType 2>/dev/null | grep -A 20 -B 5 "Mount Point:" || echo "No disks found"
        """)
        
        if profilerResult.success && !profilerResult.output.contains("No disks found") {
            let lines = profilerResult.output.components(separatedBy: "\n")
            var currentName = "Unknown"
            var currentSize = "Unknown"
            var currentMountPoint = ""
            
            for line in lines {
                if line.contains("Media Name:") || line.contains("Volume Name:") {
                    ...
                    let components = line.components(separatedBy: ":")
                    if components.count > 1 {
                        currentSize = components[1].trimmingCharacters(in: .whitespaces)
                    }
                } else if line.contains("Mount Point:") {
                    let components = line.components(separatedBy: ":")
                    if components.count > 1 {
                        currentMountPoint = components[1].trimmingCharacters(in: .whitespaces)
                        
                        // Create a drive from the collected info
                        ...
                        drives.append(DriveInfo(
                            ...
                            isInternal: !currentMountPoint.contains("Volumes"),
                            isEFI: false,
                            partitions: [],
                            isMounted: !currentMountPoint.isEmpty
                        ))
    ...
    private func parseDiskUtilText(_ output: String) -> [DriveInfo] {
        ...
        print("📄 First 10 lines of diskutil output:")
        ...
        for line in lines {
            // Look for disk header (more flexible pattern)
            if line.contains("/dev/disk") && line.contains(":") {
                ...
                if components.count > 0 {
                    // Extract disk identifier (e.g., "disk0", "disk1")
                    let diskPart = components[0].trimmingCharacters(in: .whitespaces)
                    if let range = diskPart.range(of: "disk\\d+", options: .regularExpression) {
                        currentDisk = String(diskPart[range])
                    } else {
                        currentDisk = nil
                    }
                    
                    isExternal = line.lowercased().contains("external") || 
                                 line.lowercased().contains("usb") ||
                                 line.lowercased().contains("removable")
                    currentPartitions = []
                    
                    // Try to extract size with multiple patterns
                    let sizePatterns = [
                        #"\((\d+\.?\d*\s*[A-Z]{2})\)"#,
                        #"(\d+\.?\d*\s*[A-Z]{2})\s*\(.*\)"#,
                        #"Size:\s*(\d+\.?\d*\s*[A-Z]{2})"#
                    ...
                    for pattern in sizePatterns {
                        if let range = line.range(of: pattern, options: .regularExpression) {
                            ...
                            if let matchRange = sizeText.range(of: #"\d+\.?\d*\s*[A-Z]{2}"#, options: .regularExpression) {
                                currentSize = String(sizeText[matchRange])
                            }
                            break
                        }
                    }
                    
                    print("📋 Parsed disk: \(currentDisk ?? "nil"), Size: \(currentSize), External: \(isExternal)")
                }
            }
            // Look for partition info (more flexible)
            else if line.trimmingCharacters(in: .whitespaces).hasPrefix("0:") ||
                    line.trimmingCharacters(in: .whitespaces).hasPrefix("*") ||
                    line.contains("APFS Container") ||
                    line.contains("EFI") {
                
                let cleanedLine = line.trimmingCharacters(in: .whitespaces)
                let partitionComponents = cleanedLine.components(separatedBy: .whitespaces).filter { !$0.isEmpty }
                
                if partitionComponents.count >= 3 {
                    var partitionId = ""
                    ...
                    if partitionComponents[0].contains(":") {
                        partitionId = partitionComponents[0].replacingOccurrences(of: "*", with: "")
                        partitionName = partitionComponents[1]
                        
                        if partitionComponents.count >= 4 {
                            partitionType = partitionComponents[3]
                        }
                        
                        if partitionComponents.count >= 6 {
                            partitionSize = partitionComponents[5]
                        }
                    } else if partitionComponents[0].hasPrefix("disk") {
                        partitionId = partitionComponents[0]
                        if partitionComponents.count >= 2 {
                            partitionName = partitionComponents[1]
                        }
                    }
    ...
    private func getMountInfo(for diskId: String) -> (mountPoint: String, isMounted: Bool) {
        ...
        let commands = [
            "diskutil info /dev/\(diskId) 2>/dev/null | grep 'Mount Point'",
            "mount | grep \"/dev/\(diskId) \" | awk '{print $3}'",
            "df -h | grep \"/dev/\(diskId)\" | awk '{print $NF}'"
        ]
    ...
    func checkFullDiskAccess() -> Bool {
        ...
        let hasAccess = !testResult.error.contains("Operation not permitted")
        print("🔐 Full Disk Access check: \(hasAccess ? "Granted" : "Denied")")
        if !hasAccess {
            print("⚠️ Error message: \(testResult.error)")
        }
        return hasAccess
    }
}
