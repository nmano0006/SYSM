...
struct ShellHelper {
    ...
    // MARK: - USB Boot Specific Functions - UPDATED
    ...
    static func findUSBDrives() -> [String] {
        ...
        let allDisks = runCommand("""
        diskutil list | grep -E '^/dev/disk[0-9]+ .*physical' | awk '{print $1}'
        """)
        ...
        if allDisks.success {
            ...
            for diskPath in disks {
                let diskName = diskPath.replacingOccurrences(of: "/dev/", with: "")
                
                // Check if it's external/USB
                let externalCheck = runCommand("""
                diskutil info \(diskPath) 2>/dev/null | grep -E "Internal.*No|External.*Yes|Protocol.*USB"
                """)
                
                if externalCheck.success {
                    let checkText = externalCheck.output.lowercased()
                    let isExternal = checkText.contains("external: yes") || 
                                    checkText.contains("internal: no") || 
                                    checkText.contains("protocol: usb")
                    
                    if isExternal {
                        print("Found USB/external drive: \(diskName)")
                        usbDrives.append(diskName)
                    }
                }
            }
        }
    ...
    static func mountUSBEFI() -> Bool {
        ...
        // 3. For each USB drive, look for EFI partition
        for usbDrive in usbDrives {
            ...
            if let efiPart = efiPartition {
                print("Found EFI partition on USB: \(efiPart)")
                
                // Try to mount it
                if mountPartition(efiPart) {
                    print("✅ Successfully mounted USB EFI: \(efiPart)")
                    return true
                }
            }
        }
        
        // 4. If no USB EFI found/mounted, try any EFI partition
        print("\nNo USB EFI mounted, trying any EFI partition...")
        return mountEFIPartition()
    }
    ...
    static func findEFIPartitionOnDisk(_ disk: String) -> String? {
        ...
        // Method 1: Look for EFI by type
        let efiByType = runCommand("""
        diskutil list /dev/\(disk) | grep -E 'EFI.*Boot|EFI EFI' | head -1 | awk '{print $NF}'
        """)
        
        if efiByType.success, let partition = efiByType.output.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty {
            print("Found EFI by type: \(partition)")
            return partition
        }
        
        // Method 2: Look for first s1 partition (common for EFI)
        let s1Check = runCommand("""
        diskutil list /dev/\(disk) | grep -E '^ +[0-9]+:.*s1' | head -1 | awk '{print $NF}'
        """)
        
        if s1Check.success, let partition = s1Check.output.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty {
            print("Found s1 partition: \(partition)")
            return partition
        }
        
        // Method 3: Try any partition
        let anyPartition = runCommand("""
        diskutil list /dev/\(disk) | grep -E '^ +[0-9]+:' | head -1 | awk '{print $NF}'
        """)
        
        if anyPartition.success, let partition = anyPartition.output.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty {
            print("Found any partition: \(partition)")
            return partition
        }
    ...
    static func mountPartition(_ partition: String) -> Bool {
        print("Attempting to mount partition: \(partition)")
        
        // Check if already mounted
        let mountCheck = runCommand("""
        diskutil info /dev/\(partition) 2>/dev/null | grep "Mount Point" | awk -F': ' '{print $2}'
        """)
        
        if mountCheck.success, let mountPoint = mountCheck.output.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty {
            if mountPoint != "(Not Mounted)" && !mountPoint.isEmpty {
                print("✅ Already mounted at: \(mountPoint)")
                return true
            }
        }
        
        // Try to mount with diskutil
        print("Mounting with diskutil...")
        let mountResult = runCommand("diskutil mount \(partition)", needsSudo: true)
        
        if mountResult.success {
            print("✅ diskutil mount successful")
            
            // Verify mount
            let verify = runCommand("""
            diskutil info /dev/\(partition) 2>/dev/null | grep "Mount Point" | awk -F': ' '{print $2}'
            """)
            
            if verify.success, let verifiedMount = verify.output.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty {
                if verifiedMount != "(Not Mounted)" && !verifiedMount.isEmpty {
                    print("Verified mount point: \(verifiedMount)")
                    return true
                }
            }
            return true
        } else {
            print("❌ diskutil mount failed: \(mountResult.output)")
            
            // Try alternative mount method
            print("Trying alternative mount...")
            let mountPoint = "/Volumes/EFI_\(partition.replacingOccurrences(of: "/", with: "_"))"
            
            let altResult = runCommand("""
            mkdir -p \(mountPoint) 2>/dev/null && \
            mount -t msdos /dev/\(partition) \(mountPoint) 2>&1
            """, needsSudo: true)
            
            if altResult.success && !altResult.output.lowercased().contains("error") {
                print("✅ Alternative mount successful at \(mountPoint)")
                return true
            } else {
                print("❌ Alternative mount failed: \(altResult.output)")
                // Clean up
                _ = runCommand("rmdir \(mountPoint) 2>/dev/null", needsSudo: true)
                return false
            }
        }
    }
    ...
    static func mountEFIPartition() -> Bool {
        ...
        // Get all potential EFI partitions
        let efiCandidates = runCommand("""
        diskutil list | grep -o 'disk[0-9]*s[0-9]*' | while read part; do
            type_info=$(diskutil info /dev/$part 2>/dev/null | grep -E "Type Name|Volume Type")
            if echo "$type_info" | grep -qi "efi.*apple_boot.*fat"; then
                echo "$part"
            fi
        done
        """)
        
        if !efiCandidates.success || efiCandidates.output.isEmpty {
            // If no EFI found by type, try all s1 partitions
            print("No EFI found by type, trying all s1 partitions...")
            let s1Partitions = runCommand("diskutil list | grep -o 'disk[0-9]*s1'")
            
            if s1Partitions.success {
                let partitions = s1Partitions.output.components(separatedBy: "\n").filter { !$0.isEmpty }
                print("Found s1 partitions: \(partitions)")
    ...
    static func getEFIPath() -> String? {
        ...
        // Method 1: Check /Volumes for EFI folder
        ...
        // Method 2: Check mount output
        let mountCheck = runCommand("mount | grep -E 'EFI.*msdos' | awk '{print $3}' | head -1")
        
        if !mountCheck.output.isEmpty {
            let path = mountCheck.output.trimmingCharacters(in: .whitespacesAndNewlines)
...
struct ContentView: View {
    ...
    private var efiSelectionView: some View {
        VStack(spacing: 20) {
            ...
            VStack(spacing: 12) {
                Button("Auto-Mount USB EFI (disk9s1)") {
                    mountUSBEFI()
                ...
                Button("Check Current EFI") {
                    checkEFIMount()
                    showEFISelectionView = false
                    alertTitle = "EFI Check"
                    alertMessage = efiPath != nil ? 
                        "EFI mounted at: \(efiPath!)" : 
                        "No EFI mounted. Try mounting USB EFI."
                    showAlert = true
                }
                .buttonStyle(.bordered)
                .foregroundColor(.green)
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    sudo diskutil mount disk9s1
                    
                    (Replace disk9s1 with your USB EFI partition)
                    """
                }
                showAlert = true
            }
