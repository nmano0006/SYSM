        ...
        for line in lines {
            if line.hasPrefix("/dev/disk") && line.contains(":") {
                ...
                if components.count > 0 {
                    let diskId = components[0].replacingOccurrences(of: "/dev/", with: "").trimmingCharacters(in: .whitespaces)
                    let isExternal = line.lowercased().contains("external") || line.lowercased().contains("usb")
...
struct ContentView: View {
    ...
    private func showSystemInfo() {
        ...
        info += "Total Drives: \(allDrives.count)\n"
