...
class ShellHelper {
    ...
    // Get ALL drives (mounted and unmounted) - Focus on partitions that can be mounted
    func getAllDrives() -> [DriveInfo] {
        print("🔍 Getting all drives (mounted and unmountable partitions)...")
        
        var drives: [DriveInfo] = []
        
        // Get all mounted partitions from mount command
        let mountResult = runCommand("mount")
        let mountLines = mountResult.output.components(separatedBy: "\n").filter { !$0.isEmpty }
        
        // Parse mounted volumes
        for line in mountLines {
            if line.contains("/dev/disk") && (line.contains("/Volumes/") || line.contains(" type ")) {
                let components = line.components(separatedBy: " on ")
                if components.count >= 2 {
                    let devicePart = components[0]
                    let rest = components[1]
                    
                    // Extract device identifier (e.g., disk4s4s1)
                    let deviceMatch = devicePart.components(separatedBy: "/")
                    if let deviceId = deviceMatch.last, deviceId.hasPrefix("disk") {
                        // Extract mount point
                        let mountComponents = rest.components(separatedBy: " type ")
                        let mountPoint = mountComponents.first?.trimmingCharacters(in: .whitespaces) ?? ""
                        
                        // Skip system volumes
                        if mountPoint.contains("/System/Volumes/") || mountPoint == "/" {
                            continue
                        }
                        
                        // Get more info about this partition
                        let info = getPartitionInfo(partitionId: deviceId)
                        var drive = info
                        drive.mountPoint = mountPoint
                        drive.isMounted = true
                        
                        // Add to drives list
                        if !drives.contains(where: { $0.identifier == drive.identifier }) {
                            drives.append(drive)
                        }
                    }
                }
        ...
        // Get unmounted partitions that could be mounted
        let diskutilResult = runCommand("diskutil list")
        let diskutilLines = diskutilResult.output.components(separatedBy: "\n")
        
        var currentDisk = ""
        for line in diskutilLines {
            if line.contains("/dev/disk") && line.contains("GUID") {
                // This is a disk header line
                let diskMatch = line.components(separatedBy: "/dev/")
                if diskMatch.count > 1 {
                    currentDisk = diskMatch[1].components(separatedBy: " ").first ?? ""
                }
            } else if line.contains("disk") && line.contains("s") && !line.contains("*") {
                // This might be an unmounted partition
                let components = line.components(separatedBy: " ")
                if let partitionComponent = components.first(where: { $0.hasPrefix("disk") && $0.contains("s") }) {
                    let partitionId = partitionComponent.trimmingCharacters(in: .whitespaces)
                    
                    // Skip if already in our list or if it's a system partition
                    if !drives.contains(where: { $0.identifier == partitionId }) && 
                       !partitionId.contains("EFI") &&
                       !partitionId.contains("Recovery") {
                        
                        let info = getPartitionInfo(partitionId: partitionId)
                        if info.size != "Unknown" && !info.name.isEmpty && info.name != "Not applicable" {
                            drives.append(info)
                        }
                    }
                }
        ...
        // Also look for specifically unmounted disks
        let lsResult = runCommand("ls /dev/disk* 2>/dev/null | grep -E 'disk[0-9]+s[0-9]+$' | grep -v 'disk[0-9]+$' | sort -u")
        let allPartitions = lsResult.output.components(separatedBy: "\n").filter { !$0.isEmpty }
        
        for partitionPath in allPartitions {
            let partitionId = partitionPath.replacingOccurrences(of: "/dev/", with: "")
        ...
        print("✅ Total drives/partitions found: \(drives.count)")
        return drives
    }
    
    private func getPartitionInfo(partitionId: String) -> DriveInfo {
        print("📋 Getting info for partition: \(partitionId)")
        
        // Get detailed info from diskutil
        let infoResult = runCommand("diskutil info /dev/\(partitionId) 2>/dev/null")
        
        var name = partitionId
        var size = "Unknown"
        var type = "Unknown"
        var mountPoint = ""
        ...
        for line in lines {
            if line.contains("Volume Name:") {
                ...
                if components.count > 1 {
                    volumeName = components[1].trimmingCharacters(in: .whitespaces)
                ...
                if components.count > 1 {
                    ...
                    if !mediaName.isEmpty && volumeName.isEmpty {
                        name = mediaName
                    }
                }
            }
        ...
        // If no specific type detected, try to determine
        if type == "Unknown" {
            if partitionId.contains("disk0") || partitionId.contains("disk1") || partitionId.contains("disk2") || partitionId.contains("disk3") || partitionId.contains("disk4") {
                type = "Internal"
                isInternal = true
            } else {
                type = "External"
        ...
        // Use partition ID as name if still unknown
        if name == partitionId && !volumeName.isEmpty {
            name = volumeName
        } else if name == partitionId {
        ...
        return DriveInfo(
            ...
            isEFI: partitionId.contains("EFI"),
            partitions: [],
            isMounted: isMounted,
    ...
    func mountAllExternalDrives() -> (success: Bool, message: String) {
        ...
        // Find unmounted external partitions
        let result = runCommand("""
        diskutil list | grep -E '^.*disk[0-9]+s[0-9]+' | while read line; do
            disk=$(echo "$line" | awk '{print $1}')
            if ! mount | grep -q "/dev/$disk "; then
                # Check if it's an external partition (not disk0-4)
                if [[ ! "$disk" =~ ^disk[0-4] ]]; then
                    # Get partition info
    ...
    func unmountAllExternalDrives() -> (success: Bool, message: String) {
        ...
        // Get all mounted external partitions
        let result = runCommand("""
