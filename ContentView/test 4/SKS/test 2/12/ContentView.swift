...
class ShellHelper {
    ...
    func runCommand(_ command: String, needsSudo: Bool = false) -> (output: String, error: String, success: Bool) {
        ...
        if needsSudo {
            // Improved AppleScript for password prompt
    ...
    // Get ALL drives (mounted and unmounted) - IMPROVED
    func getAllDrives() -> [DriveInfo] {
        ...
        // Now get ALL partitions from diskutil list with better parsing
        ...
        for line in lines {
            ...
            // Look for partition lines - improved detection
            if line.contains("disk") && line.contains("s") {
                // Check if this line contains a partition
                let components = line.components(separatedBy: .whitespaces).filter { !$0.isEmpty }
                ...
                for component in components {
                    if component.hasPrefix("disk") && component.contains("s") {
                        ...
                        if !drives.contains(where: { $0.identifier == partitionId }) {
                            ...
                            // Get drive info with better name detection
                            ...
                            if !isActuallyMounted {
                                ...
                                if !drive.name.contains("Recovery") && 
                                   ...
                                   drive.size != "0 B" &&
                                   !partitionId.contains("s6") && // Skip update partitions
                                   !partitionId.contains("s5") { // Skip VM partitions
    ...
    private func getDriveInfoWithName(deviceId: String, fromDiskList: [String]) -> DriveInfo {
        ...
        if (name == "Disk \(deviceId)" || name.isEmpty) && !fromDiskList.isEmpty {
            // Find the line containing this partition in diskutil list
            for line in fromDiskList {
                if line.contains(deviceId) {
                    // Try to extract name after the partition identifier
    ...
    // Mount selected drives - IMPROVED for NTFS and EFI
    func mountSelectedDrives(drives: [DriveInfo]) -> (success: Bool, message: String) {
        ...
        for drive in drives where drive.isSelectedForMount && !drive.isMounted {
            ...
            // Special handling for different file systems
            if drive.type == "EFI" || drive.type == "FAT32" {
                // EFI and FAT32 partitions often need sudo
                let result = runCommand(mountCommand, needsSudo: true)
                
                if result.success {
                    successCount += 1
                    messages.append("✅ \(drive.name): Mounted successfully (EFI/FAT32)")
                } else {
                    failedCount += 1
                    messages.append("❌ \(drive.name): Failed - \(result.error)")
                }
            } else if drive.type == "NTFS" {
                // Try different methods for NTFS
                messages.append("🔄 \(drive.name): Trying NTFS mount methods...")
                
                // Method 1: Standard mount
                var result = runCommand(mountCommand, needsSudo: true)
                
                if result.success {
                    successCount += 1
                    messages.append("✅ \(drive.name): Mounted with standard method")
                } else {
                    // Method 2: Try with read-write if possible
                    print("⚠️ Standard NTFS mount failed, trying alternative...")
                    mountCommand = "diskutil mount readOnly /dev/\(drive.identifier)"
                    result = runCommand(mountCommand, needsSudo: true)
                    
                    if result.success {
                        successCount += 1
                        messages.append("✅ \(drive.name): Mounted read-only (NTFS)")
                        messages.append("ℹ️ Note: NTFS write support requires additional software")
                    } else {
                        failedCount += 1
                        messages.append("❌ \(drive.name): Failed - macOS native NTFS support is limited")
                        messages.append("💡 Tip: Install NTFS-3G or Paragon NTFS for write support")
                    }
                }
            } else {
                // For other file systems (APFS, HFS+, etc.)
                let result = runCommand(mountCommand, needsSudo: true)
                ...
                if result.success {
                    ...
                    print("⚠️ Standard mount failed, trying mountDisk...")
            ...
            // Verify mount
            if successCount > 0 {
                let verify = runCommand("diskutil info /dev/\(drive.identifier) | grep 'Mount Point'")
                if verify.output.contains("Not applicable") {
                    messages.append("⚠️ \(drive.name): Mount reported success but not showing mount point")
                } else {
                    // Extract mount point
                    let mountPointResult = runCommand("diskutil info /dev/\(drive.identifier) | awk -F': ' '/Mount Point/ {print $2}'")
                    ...
                    if !mountPoint.isEmpty && mountPoint != "Not applicable" {
                        messages.append("📌 Mounted at: \(mountPoint)")
                    }
                }
            }
    ...
    func mountAllExternalDrives() -> (success: Bool, message: String) {
        ...
        if successCount > 0 && failedCount == 0 {
            ...
            return (true, "No unmounted drives found")
        } else {
    ...
    func unmountAllExternalDrives() -> (success: Bool, message: String) {
        ...
        if successCount > 0 && failedCount == 0 {
            ...
            return (true, "No non-system drives mounted")
        } else {
...
class DriveManager: ObservableObject {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                ...
                for drive in self.allDrives {
                    ...
                    print("   - \(drive.name) (\(drive.identifier)): \(drive.isMounted ? "📌 Mounted" : "📦 Unmounted") - Size: \(drive.size) - \(selection)")
                }
            }
        }
    }
    ...
    func mountSelectedDrives() -> (success: Bool, message: String) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
            print("🔄 Force refreshing drives after mount operation")
            self.refreshDrives()
            ...
            print("🔍 Current drive states after refresh:")
            for drive in self.allDrives {
