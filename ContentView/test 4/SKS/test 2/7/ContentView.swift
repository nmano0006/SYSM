...
class ShellHelper {
    ...
    private func getDiskInfo(diskId: String) -> DriveInfo {
        ...
        let cleanedDiskId = diskId.replacingOccurrences(of: "/dev/", with: "")
        
        // Check if disk is mounted and get mount point
        let mountCheck = runCommand("mount | grep \"/dev/\(cleanedDiskId)[\\s\\t]\" | head -1")
        let isMounted = !mountCheck.output.isEmpty
        
        var mountPoint = ""
        var name = "Disk \(cleanedDiskId)"
        var size = "Unknown"
        ...
        if isMounted {
            ...
            let parts = mountCheck.output.components(separatedBy: " on ").filter { !$0.isEmpty }
            if parts.count >= 2 {
                let mountParts = parts[1].components(separatedBy: " type ")
                if !mountParts.isEmpty {
                    mountPoint = mountParts[0]
                    ...
                    if mountPoint == "/" {
                        ...
                        name = volumeName.isEmpty ? "Disk \(cleanedDiskId)" : volumeName
            ...
            let infoResult = runCommand("diskutil info /dev/\(cleanedDiskId) 2>/dev/null | grep -E 'Size|Protocol'")
            ...
            let nameResult = runCommand("diskutil info /dev/\(cleanedDiskId) 2>/dev/null | grep 'Volume Name'")
        ...
        if !isInternal {
            ...
            diskutil info /dev/\(cleanedDiskId) 2>/dev/null | grep -E 'Protocol.*USB|Bus Protocol.*USB|Removable.*Yes' | head -1
            """)
        ...
        // Get partitions
        let partitions = getPartitionsForDisk(diskId: cleanedDiskId)
        
        return DriveInfo(
            name: name,
            identifier: cleanedDiskId,
            size: size,
            type: driveType,
    ...
    private func getPartitionsForDisk(diskId: String) -> [PartitionInfo] {
        ...
        for partitionPath in partitionIds {
            let partitionId = (partitionPath as NSString).lastPathComponent.replacingOccurrences(of: "/dev/", with: "")
            
            // Get mount point
            let mountResult = runCommand("mount | grep \"/dev/\(partitionId)[\\s\\t]\" | awk -F ' on ' '{print $2}' | awk -F ' type ' '{print $1}'")
    ...
    func mountAllExternalDrives() -> (success: Bool, message: String) {
        ...
        for disk in $(ls /dev/disk* 2>/dev/null | grep -E 'disk[0-9]+$'); do
            ...
            if ! mount | grep -q "/dev/$disk_name[[:space:]]"; then
                # Check if external
