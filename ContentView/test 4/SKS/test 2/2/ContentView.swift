...
// MARK: - Shell Helper (Fixed with proper escape sequences)
class ShellHelper {
    ...
    func debugDriveDetection() -> String {
        ...
        let test1 = runCommand("diskutil list 2>&1 | head -20")
    ...
    func getAllDrives() -> [DriveInfo] {
        ...
        // If diskutil returns empty but no error, try with sudo
        if listResult.output.isEmpty && listResult.success {
            print("⚠️ diskutil list returned empty but succeeded - trying with sudo...")
            listResult = runCommand("diskutil list", needsSudo: true)
        }
        
        // If diskutil fails or returns empty, try alternative command
        if !listResult.success || listResult.output.isEmpty {
            print("⚠️ diskutil list failed or empty, trying alternative...")
            listResult = runCommand("ls -la /dev/disk* 2>&1")
            print("Alternative output length: \(listResult.output.count)")
        }
        
        if listResult.success && !listResult.output.isEmpty {
            print("📊 Parsing diskutil output...")
            print("First 500 chars of output: \(listResult.output.prefix(500))")
            drives = parseDiskUtilText(listResult.output)
        } else {
            print("❌ Could not get disk list. Error: \(listResult.error)")
            print("Output was empty: \(listResult.output.isEmpty)")
        }
        ...
        // Fallback 2: Parse from mount command
        if drives.isEmpty {
            print("🔄 Trying mount command fallback...")
            drives = getDrivesFromMount()
        }
    ...
    private func getDrivesFromMount() -> [DriveInfo] {
        var drives: [DriveInfo] = []
    ...
    private func getDrivesFromSystemProfiler() -> [DriveInfo] {
        ...
        system_profiler SPStorageDataType -xml 2>/dev/null || system_profiler SPStorageDataType 2>/dev/null
        """)
        
        if profilerResult.success && !profilerResult.output.isEmpty {
            print("📊 Parsing system_profiler output...")
            ...
            for line in lines {
                ...
                if trimmedLine.contains("Mount Point:") {
                    ...
                    if components.count > 1 {
                        ...
                        if !currentMountPoint.isEmpty && currentMountPoint != "Not Mounted" {
                ...
                } else if trimmedLine.contains("Media Name:") || trimmedLine.contains("Volume Name:") {
                    let components = trimmedLine.components(separatedBy: ":")
                    if components.count > 1 {
                        currentName = components[1].trimmingCharacters(in: .whitespaces)
                    }
                } else if trimmedLine.contains("Capacity:") || trimmedLine.contains("Size:") {
                    let components = trimmedLine.components(separatedBy: ":")
                    if components.count > 1 {
    ...
    private func parseDiskUtilText(_ output: String) -> [DriveInfo] {
        ...
        print("📄 First 20 lines of diskutil output:")
        ...
        for line in lines {
            ...
            if trimmedLine.contains("/dev/disk") || (trimmedLine.hasPrefix("disk") && trimmedLine.contains(":")) {
                ...
                if components.count > 0 {
                    ...
                    if let range = diskPart.range(of: "disk[0-9]+", options: .regularExpression) {
                        currentDisk = String(diskPart[range])
                    } else if diskPart.hasPrefix("disk") && diskPart.count > 4 {
                        currentDisk = diskPart
                    } else {
                        currentDisk = nil
                    }
                    ...
                    // Try to extract size - FIXED ESCAPE SEQUENCES
                    let sizePatterns = [
                        #"\((\d+\.?\d*\s*[A-Z]{2})\)"#,
                        #"(\d+\.?\d*\s*[A-Z]{2})\s*\(.*\)"#,
                        #"Size:\s*(\d+\.?\d*\s*[A-Z]{2})"#,
                    ...
                    for pattern in sizePatterns {
                        if let range = trimmedLine.range(of: pattern, options: .regularExpression) {
                            let sizeText = String(trimmedLine[range])
            ...
            // Look for partition info
            else if inDiskSection && (trimmedLine.hasPrefix("0:") || 
                                     trimmedLine.contains("APFS") || 
                                     trimmedLine.contains("EFI") ||
                                     trimmedLine.contains("disk") && trimmedLine.contains("s")) {
                
                let partitionComponents = trimmedLine.components(separatedBy: .whitespaces).filter { !$0.isEmpty }
                ...
                if partitionComponents.count >= 3 {
                    ...
                    for component in partitionComponents {
                        if component.contains("disk") && component.contains("s") {
                            partitionId = component
                            break
                        }
                    }
                    
                    // Find partition name (usually after identifier)
                    if let idIndex = partitionComponents.firstIndex(where: { $0 == partitionId }),
                       idIndex + 1 < partitionComponents.count {
                        partitionName = partitionComponents[idIndex + 1]
                    }
                    ...
                    currentPartitions.append(PartitionInfo(
                        name: partitionName,
                        identifier: partitionId.isEmpty ? "unknown" : partitionId,
                        size: partitionSize,
                        type: partitionType,
                        mountPoint: "",
            ...
            // End of disk section
            else if inDiskSection && trimmedLine.isEmpty {
                inDiskSection = false
            }
        }
...
// MARK: - Data Structures (FIXED TYPOS: DriveInfo not Drivelnfo)
