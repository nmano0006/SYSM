...
class ShellHelper {
        ...
        for line in dfLines {
            ...
            if devicePath.hasPrefix("/dev/disk") && 
               ...
               !mountPoint.contains("Library/Developer") {
                ...
                // Skip system partitions like disk4s1, disk4s2, etc. (usually system disks)
                if deviceId.starts(with: "disk4") || deviceId.starts(with: "disk0") {
                    // Only include if it's in /Volumes/ (user volume)
                ...
                let updatedDrive = DriveInfo(
                    ...
                    isEFI: deviceId.contains("EFI") || volumeName == "EFI",
                ...
                print("📌 Found mounted user volume: \(updatedDrive.name) (\(deviceId)) at \(mountPoint)")
            }
        }
    ...
    private func getUnmountedPartitions() -> [DriveInfo] {
        ...
        // Get list of all partitions from diskutil
        let listResult = runCommand("""
        diskutil list | grep -E 'disk[0-9]+s[0-9]+' | awk '{print $1}'
        """)
        
        let partitionIds = listResult.output.components(separatedBy: "\n").filter { !$0.isEmpty }
        
        for partitionId in partitionIds {
            // Skip if we've already processed this as mounted
            if partitions.contains(where: { $0.identifier == partitionId }) {
                continue
            }
            
            // Skip system partitions (disk0-4, which are usually internal system disks)
            if partitionId.starts(with: "disk0") || partitionId.starts(with: "disk1") || 
               partitionId.starts(with: "disk2") || partitionId.starts(with: "disk3") || 
               partitionId.starts(with: "disk4") {
                // Only include if it's not a system volume (like Recovery, VM, etc.)
                let drive = getDriveInfo(deviceId: partitionId)
                if drive.name.contains("Recovery") || drive.name.contains("VM") || 
                   drive.name.contains("Preboot") || drive.name.contains("Update") {
                    continue
                }
            }
            ...
            if !isMounted {
                ...
                // Include all partitions, even if they don't have fancy names
                // But skip Apple_APFS_ISC if it's empty
                if drive.name == "Apple_APFS_ISC" && drive.size.contains("0 B") {
                    continue
                }
                ...
                print("📌 Found unmounted: \(drive.name) (\(partitionId))")
            }
        }
    ...
    private func getDriveInfo(deviceId: String) -> DriveInfo {
        ...
        for line in lines {
            ...
            } else if line.contains("Type (Bundle):") {
                let components = line.components(separatedBy: ":")
                if components.count > 1 {
        ...
        if type == "Unknown" {
            if deviceId.contains("EFI") {
                type = "EFI"
            } else if deviceId.starts(with: "disk0") || deviceId.starts(with: "disk1") || 
        ...
        return DriveInfo(
            ...
            isEFI: deviceId.contains("EFI") || type == "EFI",
            partitions: [],
            isMounted: isMounted,
            isSelectedForMount: false,
            isSelectedForUnmount: false
    ...
    func mountAllExternalDrives() -> (success: Bool, message: String) {
        ...
        for disk in $(diskutil list | grep -E 'disk[0-9]+s[0-9]+' | awk '{print $1}'); do
