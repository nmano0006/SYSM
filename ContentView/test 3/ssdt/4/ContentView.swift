...
// MARK: - Enhanced SSDT Generator View with USB Support
@MainActor
struct SSDTGeneratorView: View {
    ...
    @State private var useFWHD = true
    @State private var useMEM2 = true
    @State private var useTMR = true
    @State private var useHDEF = true
    ...
    let motherboardModels = [
        ...
        "Dell OptiPlex 7090", "Dell OptiPlex 9010", "Dell Precision T3610",
        "Dell Precision T5810", "Dell Precision T7910", "Dell XPS 8930",
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    HStack {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            } else if selectedDeviceType == "Audio" {
                                VStack(alignment: .leading, spacing: 4) {
                                    Text("Audio Codec")
                                        .font(.caption)
                                        .foregroundColor(.secondary)
    ...
    private func generateFromPreset() {
        ...
        // Auto-select based on motherboard preset
        applyMotherboardPreset(selectedMotherboardPreset)
        
        // Generate after a short delay to show progress
    ...
    private func applyMotherboardPreset(_ preset: String) {
        ...
        case "AMD X670", "AMD X570", "AMD X470":
            ...
            usePLUG = true
            useXOSI = true
            useAWAC = false  // Not needed for AMD
            useHDEF = true
        ...
        default:
            // For "Auto" or "Custom", use sensible defaults
        ...
        // Update motherboard model if a specific one matches the preset
    ...
    private func generateUSBSSDT() {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            // Generate USB power properties SSDT
            if self.usbPowerProperties {
                let usbxContent = self.generateUSBX()
                let usbxPath = "\(outputDir)/SSDT-USBX.dsl"
                
                do {
                    try usbxContent.write(toFile: usbxPath, atomically: true, encoding: .utf8)
                    DispatchQueue.main.async {
                        self.generatedSSDTs.append("SSDT-USBX.dsl")
                    }
                    
                    if self.includeCompilation {
                        let result = self.compileDSLToAML(dslPath: usbxPath, amlPath: usbxPath.replacingOccurrences(of: ".dsl", with: ".aml"))
                        if result.success {
                            compilationMessages.append("✅ SSDT-USBX: Compiled successfully")
                            DispatchQueue.main.async {
                                self.generatedSSDTs.append("SSDT-USBX.aml")
                            }
                        } else {
                            compilationMessages.append("⚠️ SSDT-USBX: Compilation failed")
                        }
                    }
                } catch {
                    compilationMessages.append("❌ SSDT-USBX: Failed to create file")
                }
            }
            
            // Generate USB port mapping SSDT based on selected port count
            if let portCount = Int(self.selectedUSBPortConfig) {
                let usbSSDTContent = self.generateUSBPortSSDT(portCount: portCount)
                ...
                do {
                    ...
                    DispatchQueue.main.async {
                        self.generatedSSDTs.append("\(ssdtName).dsl")
                    }
                    
                    if self.includeCompilation {
                        let result = self.compileDSLToAML(dslPath: ssdtPath, amlPath: ssdtPath.replacingOccurrences(of: ".dsl", with: ".aml"))
                        if result.success {
                            compilationMessages.append("✅ \(ssdtName): Compiled successfully")
                            DispatchQueue.main.async {
                                self.generatedSSDTs.append("\(ssdtName).aml")
                            }
                        } else {
                            compilationMessages.append("⚠️ \(ssdtName): Compilation failed")
                        }
                    }
                } catch {
                    compilationMessages.append("❌ \(ssdtName): Failed to create file")
                }
            }
            
            // Generate UIAC for port mapping
            if self.usbInjector {
                let uiacContent = self.generateUIAC()
                let uiacPath = "\(outputDir)/SSDT-UIAC.dsl"
                
                do {
                    try uiacContent.write(toFile: uiacPath, atomically: true, encoding: .utf8)
                    DispatchQueue.main.async {
                        self.generatedSSDTs.append("SSDT-UIAC.dsl")
                    }
                } catch {
                    compilationMessages.append("❌ SSDT-UIAC: Failed to create file")
                }
            }
            
            // Generate GPRW for USB wake fix
            if self.usbWakeFix {
                let gprwContent = self.generateGPRW()
                let gprwPath = "\(outputDir)/SSDT-GPRW.dsl"
                
                do {
                    try gprwContent.write(toFile: gprwPath, atomically: true, encoding: .utf8)
                    DispatchQueue.main.async {
                        self.generatedSSDTs.append("SSDT-GPRW.dsl")
                    }
                } catch {
                    compilationMessages.append("❌ SSDT-GPRW: Failed to create file")
                }
            }
            ...
            DispatchQueue.main.async {
                ...
                self.compilationResult = compilationMessages.joined(separator: "\n")
                
                self.alertTitle = "USB SSDTs Generated"
                self.alertMessage = """
                Successfully generated \(self.generatedSSDTs.count) USB SSDTs for \(self.selectedUSBPortConfig) ports.
                
                • SSDT-USBX: USB Power Properties
                • SSDT-XHC\(self.selectedUSBPortConfig): \(self.selectedUSBPortConfig)-port configuration
    ...
    private func generateSSDTs() {
        ...
        // If USB device type is selected, generate USB SSDTs
        if selectedDeviceType == "USB" {
            generateUSBSSDT()
            return
        }
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            for (index, ssdt) in ssdtsToGenerate.enumerated() {
                ...
                do {
                    ...
                    DispatchQueue.main.async {
                        self.generatedSSDTs.append(dslFilename)
                    }
                    ...
                    if self.includeCompilation {
                        ...
                        if result.success {
                            ...
                            DispatchQueue.main.async {
                                self.generatedSSDTs.append(amlFilename)
                            }
                        } else {
            ...
            DispatchQueue.main.async {
                ...
                self.alertTitle = "SSDTs Generated"
                self.alertMessage = """
                Successfully generated \(self.generatedSSDTs.count) files for \(self.motherboardModel):
                
                • DSL source files: \(ssdtsToGenerate.count)
                • AML binary files: \(self.includeCompilation ? "\(compilationMessages.filter { $0.contains("✅") }.count)" : "Compilation disabled")
    ...
    private func generateUSBPortSSDT(portCount: Int) -> String {
        ...
        // Calculate root hub depth based on port count
    ...
    private func generateValidDSLContent(for ssdt: String) -> String {
        // Use the templates from the provided SSDT.swift file
        switch ssdt {
        case "SSDT-DTGP":
            return generateDTGP()
        case "SSDT-ALS0":
    ...
    private func validateSSDTs() {
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                self.alertTitle = "SSDT Validation"
                self.alertMessage = validationMessages.joined(separator: "\n")
                self.showAlert = true
            }
        }
    }
    ...
    private func installToEFI() {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                self.alertTitle = "EFI Installation"
                ...
                self.alertMessage = installMessages.joined(separator: "\n")
                self.showAlert = true
            }
        }
    }
