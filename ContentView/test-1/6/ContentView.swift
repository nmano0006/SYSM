...
struct ShellHelper {
    ...
    static func mountEFIPartition() -> Bool {
        ...
        // First, list disks to find EFI partition
        ...
        // Find EFI partition (usually disk0s1 or similar)
        let findEFIResult = runCommand("""
        diskutil list | grep -E 'EFI|EFI.*Boot' | head -1 | awk '{print $NF}'
        """)
        
        guard findEFIResult.success, let efiIdentifier = findEFIResult.output.split(separator: "\n").first else {
            print("Could not find EFI partition")
            return false
        }
        
        let efiID = String(efiIdentifier).trimmingCharacters(in: .whitespacesAndNewlines)
        
        if efiID.isEmpty {
            print("EFI identifier is empty")
            return false
        }
        
        print("Found EFI partition: \(efiID)")
        
        // Check if already mounted
        let checkMountedResult = runCommand("mount | grep '\(efiID)'")
        if checkMountedResult.success && !checkMountedResult.output.isEmpty {
            print("EFI partition already mounted")
            return true
        }
        ...
        if mountResult.success {
            print("Successfully mounted EFI partition")
            return true
        } else {
            print("Failed to mount EFI: \(mountResult.output)")
            return false
        }
    }
    ...
    static func getEFIPath() -> String? {
        ...
        mount | grep -E '/dev/disk.*s1' | grep -v 'VirtualBox' | grep -v 'VMware' | awk '{print $3}' | head -1
        """)
        ...
        diskutil list | grep -B2 'EFI' | grep -o '/dev/disk[0-9]*s[0-9]*' | head -1 | xargs -I {} diskutil info {} | grep 'Mount Point' | awk '{print $3}'
        """)
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                if systemProtectStatus == "Disabled" && efiPath != nil {
                    NavigationLink(destination: KextManagementView(
                        isInstallingKext: .constant(false),
                        showAlert: $showAlert,
                        alertTitle: $alertTitle,
                        alertMessage: $alertMessage,
                        appleHDAStatus: $appleHDAStatus,
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                ...
                if success && path != nil {
                    ...
                    alertTitle = "Error"
                    alertMessage = "Failed to mount EFI partition. Please mount it manually."
                }
                showAlert = true
            }
        }
